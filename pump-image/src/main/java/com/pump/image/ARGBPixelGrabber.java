 ...
 * <code>BufferedImage</code>'s from becoming un-managed.
 * <P>
 * If this is asked to iterate over the image data for a non-supported
 * <code>Image</code>, then this class will defer to a <code>PixelGrabber</code>.
 * <P>
...
@Blurb(title = "Performance: Studying the PixelGrabber", releaseDate = "April 2007", summary = "Can we improve on the <a href=\"http://java.sun.com/javase/6/docs/api/index.html?java/awt/image/PixelGrabber.html\"><code>java.awt.PixelGrabber</code></a>?\n"
		...
		+ "you might be able to really improve performance with the <code><a href=\"https://javagraphics.java.net/doc/com/bric/image/ARGBPixelGrabber.html\">ARGBPixelGrabber</a></code>.", article = "http://javagraphics.blogspot.com/2007/04/pixelgrabber-studying-performance.html")
public class ARGBPixelGrabber {
	...
	public static void main(String[] args) {
		...
		Arrays.sort(t);
		System.out.println("PixelGrabber.grabPixels: " + t[t.length / 2]
				+ " ms");
		...
		Arrays.sort(t);
		System.out.println("ARGBPixelGrabber.grabPixels: " + t[t.length / 2]
				+ " ms");
	 ...
	 * 
	 * @param rect
	 *            if non-null, then this object will only iterate over this
	 *            rectangle. An exception may be thrown if <code>rect</code> is
	 *            not completely contained within <code>i</code>
	 */
	public ARGBPixelGrabber(Image i, Rectangle rect) {
		if (i instanceof BufferedImage
			...
			if (rect == null) {
				...
				if (rect.x < 0 || rect.y < 0 || (rect.x + rect.width > imageW)
						|| (rect.y + rect.height > imageH)) {
					System.err.println("Image size: " + imageW + " x " + imageH
							+ " pixels");
			...
			if (rect == null) {
				...
				pg = new PixelGrabber(i, rect.x, rect.y, rect.width,
						rect.height, true);
			}
			String errmsg = null;
			try {
	 ...
	 * <P>
	 * This method is provided largely to be consistant with the
	 * <code>java.awt.image.PixelGrabber</code>. If it is possible, it is
	 * probably more efficient to iterate over the pixel data one row at a time.
	 * <P>
	 * This method returns the entire image in bulk, and does not interact with
	 * the "iterator" nature of this object. So calling this method will not
	 * affect the state of <code>isDone</code>, or the state of future calls to
	 * <code>next(...)</code>.
	 * <P>
	 * Calling this method is equivalent to calling: <BR>
	 * <code>getPixels(new int[getWidth()*getHeight()]);</code>
	 * 
	 ...
	 * 
	 * @param array
	 *            this is the destination array to store this data in. If this
	 *            is <code>null</code> then a new array will be created for you.
	 *            An exception will be thrown if the array is not large enough
	 *            to accomodate all the pixel data.
	...
	public int[] getPixels(int[] array) {
		if (array == null) {
			...
			if (array.length < width * height)
				throw new IllegalArgumentException(
						"This array is not big enough to fit all the image data present.  ( "
	 ...
	 * (Unless the end of the image is reached.)
	 * <P>
	 * This is equivalent to calling: <BR>
	 * <code>next(data,0,data.length);</code>
	 * 
	 * @param data
	 ...
	 * 
	 * @param data
	 *            the array to fill. This array can be of arbitrary length, but
	 *            you may see better performance if you ask for
	 *            <code>getWidth()</code> many pixels at a time.
	 * @param arrayOffset
	 *            this is the index to begin writing to in <code>data</code>.
	 *            <P>
	 *            If the image involved is a <code>BufferedImage</code> of type
	 *            <code>ARGB_PRE</code>, <code>ARGB</code> or <code>RGB</code>
	 *            then you will see improved performance if this value is zero.
	 * @param arrayLength
	 *            this is the amount of data that should be read into
	 *            <code>data</code>. An exception will be thrown if this method
	 *            is asked to write more pixel data than will fit in the array.
	 *            <P>
	 *            If this value is a non-positive number, then an arbitrary
	 *            number of pixels will be stored in the array.
	 * @return the number of pixels written in this array. This will generally
	 *         be equal to <code>arrayLength</code>, except in 2 cases:
	 *         <ul>
	 *         <li>When this iterator has finished.</li> <li>When <code>
	 *         arrayLength</code> is non-positive. In this case this object gets
	 *         to decide how many pixels to return at a time.</li>
	 *         </ul>
	 *         <P>
	 *         This will only return 0 if <code>isDone()</code> returns
	 *         <code>true</code>.
	 */
	public int next(int[] data, int arrayOffset, int arrayLength) {
		if (arrayLength > 0) {
			if (data.length - arrayOffset < arrayLength)
				throw new IllegalArgumentException("Illegal request for "
						+ arrayLength
						+ " pixels, when the array can only contain "
						+ (data.length - arrayOffset) + " pixels.");
		} else {
		...
		while (!(isDone()) && copied < arrayLength) {
			if (currentX == x && arrayLength - copied >= width) {
				...
				System.arraycopy(row, currentX - x, data, arrayOffset + copied,
						amt);
				currentX += amt;
				copied += amt;
				if (currentX == width) {
	 ...
	 * @param destOffset
	 *            the offset in dest to write to
	 */
	private int[] readRow(int y, int[] dest, int destOffset) {
		if (dest.length - destOffset < width)
			throw new IllegalArgumentException("this array must have at least "
					+ width + " pixels available in it.  ("
		...
		if (r != null) {
			...
			case BufferedImage.TYPE_INT_ARGB_PRE:
				if (destOffset == 0) {
					...
					System.arraycopy(scratchIntArray, 0, dest, destOffset,
							width);
				}
			...
			case BufferedImage.TYPE_3BYTE_BGR:
				...
				for (int a = 0; a < width; a++) {
					dest[destOffset + a] = ((scratchByteArray[3 * a + 2] & 0xFF) << 16)
							+ // red
			...
			case BufferedImage.TYPE_4BYTE_ABGR_PRE:
				...
				r.getDataElements(x, y, width, 1, scratchByteArray);
				for (int a = 0; a < width; a++) {
					dest[destOffset + a] = ((scratchByteArray[4 * a] & 0xFF) << 24)
							+ // alpha
			...
			case BufferedImage.TYPE_BYTE_INDEXED:
				...
				for (int a = 0; a < row.length; a++) {
					...
					dest[destOffset + a] = (icm.getRed(t2) << 16)
							+ (icm.getGreen(t2) << 8) + icm.getBlue(t2) + alpha
							* (0xff000000);
				}
				break;
			default:
