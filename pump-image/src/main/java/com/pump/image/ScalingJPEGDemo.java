...
/**
 * This app uses a large sample JPEG (a few thousand pixels wide) and applies
 * different scaling techniques.
 * <p>
 * The original incentive for this comparison was to test the
 * GenericImageSinglePassIterator class.
 *
 * TODO: this is currently in the image project, but it should be in the
 * showcase project
 */
public class ScalingJPEGDemo {
	...
	static abstract class ScalingReader {
		...
		public abstract Image create(Dimension originalSize, Dimension maxSize)
				throws Throwable;
	...
	public static URL getImageURL() {
		// try changing this to "resources/bridge2.jpg" (a 15000-pixel wide
		// image)
	...
	public static void main(String[] args) {
		BasicConsole console = BasicConsole.create("ScalingJPEGDemo", false,
				true, true);
		...
		try {
			...
			ScalingReader singlePassReader = new ScalingReader() {
				public Image create(Dimension originalSize, Dimension maxSize)
						throws Throwable {
					URL url = getImageURL();
					return GenericImageSinglePassIterator.createScaledImage(
							url, maxSize);
				}
			...
			ScalingReader scalingGraphicsReader = new ScalingReader() {
				...
				public Image create(Dimension originalSize, Dimension maxSize)
						throws Throwable {
					Dimension newSize = Dimension2D.scaleProportionally(
					...
					BufferedImage thumbnail = new BufferedImage(newSize.width,
			...
			ScalingReader transformReader = new ScalingReader() {
				...
				public Image create(Dimension originalSize, Dimension maxSize)
					...
					BufferedImage thumbnail = new BufferedImage(newSize.width,
							newSize.height, BufferedImage.TYPE_INT_ARGB);
					Graphics2D g = thumbnail.createGraphics();
					double scale = ((double) newSize.width)
			...
			ScalingReader scaledInstanceReader = new ScalingReader() {
				...
				public Image create(Dimension originalSize, Dimension maxSize)
					...
					Image image2 = image.getScaledInstance(newSize.width,
							newSize.height, Image.SCALE_SMOOTH);
			...
			ScalingReader replicateScaleFilter = new ScalingReader() {
				...
				public Image create(Dimension originalSize, Dimension maxSize)
					...
					ImageFilter scalingFilter = new ReplicateScaleFilter(
							newSize.width, newSize.height);
					ImageProducer scaledProducer = new FilteredImageSource(
							producer, scalingFilter);
			...
			ScalingReader areaAveragingScaleFilter = new ScalingReader() {
				...
				public Image create(Dimension originalSize, Dimension maxSize)
					...
					ImageProducer producer = image.getSource();
					ImageFilter scalingFilter = new AreaAveragingScaleFilter(
							newSize.width, newSize.height);
					ImageProducer scaledProducer = new FilteredImageSource(
							producer, scalingFilter);
					Image image2 = Toolkit.getDefaultToolkit().createImage(
			...
			/**
			 * Note the following subsampling model isn't really accurate. If
			 * you look at the exported images: the rounding involved causes
			 * some severe problems.
			 * 
			 * Also: as of March of 2015 this reader will cause all readers to
			 * show the error "Too many open files in system". Apple may have
			 * reduced the open file limit, but this is still a scary error. (
			 * http://superuser.com/questions/433746/is-there-a-fix-for-the-too-
			 * many-open-files-in-system-error-on-os-x-10-7-1 )
			 *
			 * Does ImageIO just not close resources correctly? Am I doing
			 * something wrong? Creepy stuff.
			 */
			List<ImageReader> ioReaderList = getIOReaderList();
			for (int a = 0; a < ioReaderList.size(); a++) {
				/**
				 * Arg. The first implementation just used the same ImageReader
				 * hundreds of times, but apparently that results in too many
				 * open files for the file system to manage. (Observed on Mac
				 * 10.10. This even affected other apps trying to open files.)
				 * 
				 * So this approach called reader.dispose() constantly, which
				 * means we have to access the ImageReader by its index in the
				 * list.
				 */
				...
				ScalingReader imageIOReader = new ScalingReader() {
					public Image create(Dimension originalSize,
							Dimension maxSize) throws Throwable {
						URL url = getImageURL();
						Dimension newSize = Dimension2D.scaleProportionally(
								originalSize, maxSize);
						...
						try (InputStream in = url.openStream();
								ImageInputStream imageIn = ImageIO
			...
			for (int a = 0; a < readers.size(); a++) {
				...
				PrintStream ps = reader.printStream == null ? out
						: reader.printStream;
			...
			for (int size = 100; size <= 2000; size += 100) {
				...
				for (int a = 0; a < readers.size(); a++) {
					...
					try {
						...
						for (int b = 0; b < time.length; b++) {
							...
							if (!(w == maxSize.width || h == maxSize.height))
								throw new RuntimeException(
										"thumbnail is not the correct size: "
												+ w + "x" + h);
							...
							/**
							 * The AWT "Image Fetcher" threads need some time to
							 * die. And while we're waiting: let's occasionally
							 * run gc.
							 * 
							 * (When I only called finalization/gc once with no
							 * sleeping: my memory results had HUGE variations.
							 * Adding this pause and the repeated gc calls made
							 * the performance numbers much more
							 * linear/explainable.
							 * 
							 */
							for (int c = 0; c < 5; c++) {
						...
						memoryResults.put(memoryResultsKey,
								format.format(medianMemory));
					} catch (Throwable t) {
						errors.add(t);
	...
	private static List<ImageReader> getIOReaderList() {
		Iterator<ImageReader> ioReaders = ImageIO
