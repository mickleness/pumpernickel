...
public class BrushedMetalLook {
	...
	/**
	 * Return a 200x200 tiling image with horizontal brushed metal tinted to a
	 * particular color.
	 * 
	 * @param color
	 *            if null then the default gray is returned.
	 * @return a 200x200 image with horizontal brushed metal tinted to a
	 *         particular color.
	 */
	public static BufferedImage getImage(Color color) {
		try {
			BufferedImage bi = ImageIO.read(BrushedMetalLook.class
					.getResource("grayMetal.png"));
			if (color == null)
				return bi;
			BufferedImage bi2 = new BufferedImage(bi.getWidth(),
					bi.getHeight(), BufferedImage.TYPE_INT_RGB);
			IntRGBConverter iter = new IntRGBConverter(
					BufferedImageIterator.get(bi));
			...
			while (!iter.isDone()) {
				...
				for (int x = 0; x < w; x++) {
					...
					rgb[x] = 0xff000000 + (newRed << 16) + (newGreen << 8)
							+ (newBlue);
				}
	...
	/**
	 * @param includeShadowsAndHighlights
	 *            add a bevel around the perimeter of the shape
	 */
	public static BufferedImage paint(Shape shape, float width,
			Rectangle imageBounds, Color color,
			boolean includeShadowsAndHighlights) {
		...
		BasicStroke mainStroke = new BasicStroke(width + 2,
		...
		BufferedImage bi = new BufferedImage(imageBounds.width,
				imageBounds.height, BufferedImage.TYPE_INT_ARGB_PRE);
		...
		for (float a = 0; a < width / 2; a += .5f) {
			for (float theta = (float) (Math.PI / 2); theta < 2 * Math.PI; theta += Math.PI) {
				...
				for (float k = 0; k < ms.getOriginalDistance();) {
					...
					Shape segment = ms.getShape(k / ms.getOriginalDistance(),
							streakDistance / ms.getOriginalDistance());

					GeneralPathWriter writer = new GeneralPathWriter(
							insetSegment);
					InsetPathWriter insetWriter = new InsetPathWriter(writer,
							a, theta);
		...
		BasicStroke otherStroke = new BasicStroke(width, BasicStroke.CAP_BUTT,
