	...
	public synchronized boolean setSegment(int pathIndex,int segmentIndex,int type,double[] coords) {
		...
		if( oldType==PathIterator.SEG_MOVETO && type!=PathIterator.SEG_MOVETO) {
			...
			if(pathIndex==0) {
				...
				List<MutablePathSegment> previousSegments = paths.get(pathIndex-1);
...
class MutablePathIterator implements PathIterator {
	...
	public MutablePathIterator(List<List<MutablePathSegment>> pathList,int windingRule,AffineTransform transform,Object syncLock) {
		winding = windingRule;
		if(transform!=null) {
