 ...
 * location to another.
 * <P>
 * This is achieved by calling: <BR>
 * <code>myComponent.putClientProperty(DESTINATION, newDestination)</code>
 * <P>
 * This layout then nudges the component until it reaches that destination. The
 * nudge is proportional to the distance that the component has to cover, giving
 * the impression of a decelerating animation as the object approaches its
 * destination.
 *
 */
public class AnimatedLayout implements LayoutManager2 {
	/**
	 * This is the delay, in milliseconds, between adjustments. I find 30 to be
	 * almost "too fast" and 50 almost "too slow".
	 */
	...
	private static void registerContainer(Container c) {
		synchronized (AnimatedLayout.class) {
			ContainerListener l = (ContainerListener) ((JComponent) c)
					.getClientProperty("animatedLayout.containerListener");
			if (l == null) {
				...
				((JComponent) c).putClientProperty(
						"animatedLayout.containerListener", l);
	...
	private static void registerChildren(JComponent c) {
		for (int a = 0; a < c.getComponentCount(); a++) {
			...
			if (child.getClientProperty("animatedLayout.propertyListener") == null) {
				child.putClientProperty("aniamtedLayout.propertyListener",
						destinationListener);
				child.addPropertyChangeListener(DESTINATION,
						destinationListener);
			}
		}
	}
