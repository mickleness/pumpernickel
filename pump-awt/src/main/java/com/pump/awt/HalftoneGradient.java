 ...
 * <P>
 * This creates an image as a tile and uses an underlying TexturePaint to do
...
public class HalftoneGradient extends TransformedTexturePaint {
	...
	/**
	 * Create a new <code>HalftoneGradient</code> using circles and the default
	 * width.
	 * 
	 * @param p1
	 *            the first point.
	 ...
	 * @param type
	 *            the type (TYPE_DIAMOND, TYPE_CIRCLE or TYPE_TRIANGLE)
	 * @param cycle
	 *            whether this gradient repeats or not.
	 * @param offset
	 *            a float from [0,1] that can be used to animate this gradient.
	 * @param shear
	 *            a float (default should be zero) that shears the transform. In
	 *            general this should be between [-2,2] for best results.
	 */
	public HalftoneGradient(Point2D p1, Color c1, Point2D p2, Color c2,
			int width, int type, boolean cycle, float offset, float shear) {
		this((float) p1.getX(), (float) p1.getY(), c1, (float) p2.getX(),
				(float) p2.getY(), c2, width, type, cycle, offset, shear);
	}
	 ...
	 *            general this should be between [-2,2] for best results.
	 */
	public HalftoneGradient(float x1, float y1, Color c1, float x2, float y2,
			Color c2, int width, int type, boolean cycle, float offset,
			float shear) {
		super(createImage(x1, y1, c1, x2, y2, c2, width, type, cycle, offset),
				createRectangle(x1, y1, x2, y2, width, cycle), createTransform(
						x1, y1, x2, y2, shear));
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x2;
	...
	public PaintContext createContext(ColorModel cm, Rectangle deviceBounds,
		...
		if (QUALITY_HINTS == null) {
			QUALITY_HINTS = new RenderingHints(
					RenderingHints.KEY_INTERPOLATION,
					RenderingHints.VALUE_INTERPOLATION_BILINEAR);
			QUALITY_HINTS.put(RenderingHints.KEY_ANTIALIASING,
					RenderingHints.VALUE_ANTIALIAS_ON);
			QUALITY_HINTS.put(RenderingHints.KEY_RENDERING,
					RenderingHints.VALUE_RENDER_QUALITY);
		}

		PaintContext context = super.createContext(cm, deviceBounds,
				userBounds, xform, QUALITY_HINTS);
		...
		context = new CoveredContext(context, x1, y1, color1, x2, y2, color2,
				newCopy);
		return context;
	}

	/** A convenience method to define a diamond centered around a fixed point. */
	private static void defineDiamond(GeneralPath path, float centerX,
			float centerY, float r) {
	...
	private static void defineTriangle(GeneralPath path, float centerX,
			float centerY, float r) {
		path.moveTo(centerX, centerY - r);
		path.lineTo(centerX + r, centerY);
		path.lineTo(centerX - r, centerY);
	...
	private static BufferedImage createImage(float x1, float y1, Color c1,
	...
	/**
	 * Paints a halftone effect from [0,0] to [0,height], where the tiled image
	 * is w-pixels wide.
	 * <P>
	 * When g is transformed correctly this can be used to transform backwards
	 * as well as forwards.
	 * 
	 * @param g
	 * @param path
	 ...
	 * @param w
	 */
	private static void paintOneSide(Graphics2D g, GeneralPath path, int type,
			int height, float offset, int w) {
		path.reset();
		...
		if (type == TYPE_TRIANGLE) {
			...
			throw new IllegalArgumentException(
					"type must be TYPE_CIRCLE or TYPE_DIAMOND");
		}
	}
	...
	private static AffineTransform createTransform(float x1, float y1,
