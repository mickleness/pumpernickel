 ...
 * <P>
 * Also there is a bug when clipping shapes using Quartz on Mac: a GeneralPath
 * encapsulating exactly the same area as a Rectangle2D may not clip correctly.
 * If this abstract shape is instead converted to a Rectangle2D: the bug goes
 * away!
 */
public class RectangleReader {
	...
	/**
	 * This studies a shape and determines if it is a <code>Rectangle</code>, a
	 * <code>Rectangle2D</code>, or neither.
	 * 
	 * @param shape
	 *            the shape to study
	 * @return a <code>Rectangle</code>, <code>Rectangle2D</code>, or
	 *         <code>null</code>.
	 */
	 ...
	 *            the shape to study
	 * @param transform
	 *            the optional transform to apply to the shape.
	 * @return a <code>Rectangle</code>, <code>Rectangle2D</code>, or
	 *         <code>null</code>.
	 */
	public static final Rectangle2D convert(Shape shape,
		...
		/*
		 * Lots of ways we could approach this... This is a straight-forward
		 * logical approach that could probably stand to be improved
		 * performance- wise: 1. Get the bounds of the shape. 2. Iterate over
		 * the shape a second time, and see if all points are collinear with the
		 * bounds.
		 */
		...
		while (i.isDone() == false) {
			...
			if (k == PathIterator.SEG_MOVETO) {
				...
				if (data[1] == top) {
					if (SimplifiedPathIterator.collinear(left, top, right, top,
							data[0], data[1]) == false) {
						return null;
					}
				} else if (data[1] == bottom) {
					if (SimplifiedPathIterator.collinear(left, bottom, right,
							bottom, data[0], data[1]) == false) {
						return null;
					}
				} else if (data[0] == left) {
					if (SimplifiedPathIterator.collinear(left, top, left,
							bottom, data[0], data[1]) == false) {
						return null;
					}
				} else if (data[0] == right) {
					if (SimplifiedPathIterator.collinear(right, top, right,
							bottom, data[0], data[1]) == false) {
						return null;
					}
				} else {
	...
	/**
	 * This checks to see if a Rectangle2D can be expressed as a int-based
	 * Rectangle.
	 * 
	 * @param r
	 * @return a new <code>Rectangle</code> if possible, or the original
	 *         argument if not.
	 */
	 ...
	 * 
	 * @param r
	 * @return a new <code>Rectangle</code> if possible, or the null if not.
	 */
	private static final Rectangle getRectangle(double x, double y, double w,
			double h) {
		if (w < 0) {
			x = x + w;
			w = -w;
