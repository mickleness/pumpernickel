...
/**
 * This is the <code>PathIterator</code> that iterates over a BasicShape.
 * <P>
 */
class BasicShapeIterator implements PathIterator {
	...
	/**
	 * This clones our own copy of the shape data if we haven't already done so.
	 * <p>
	 * This method is <i>not</i> called by default, because ideally we'd like to
	 * not clone all those arrays as we iterate over the shape. However if the
	 * segments we're referencing need to change (because the BasicShape is
	 * somehow being altered), then this method will be called.
	 */
