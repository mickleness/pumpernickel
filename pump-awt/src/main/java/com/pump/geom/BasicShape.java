...
public class BasicShape implements Shape {
	 ...
	 *            the second value
	 * @param tolerance
	 *            the acceptable tolerance. Must be positive.
	...
	/**
	 * This lists an array of double values as a String. Mostly used for
	 * debugging.
	 * 
	 * @param array
	 * @param offset
	...
	/**
	 * These are WeakReferences to PathIterators that are relying on THIS object
	 * to iterate through data. If we're asked to transform, or add, or clip, or
	 * somehow modify this object's shape data, we need to grab these iterators
	 * and give them our cloned path data so they don't need this object
	 * anymore.
	 */
	 ...
	 * 
	 * <p>
	 * If the shape is a simple shape in the <code>java.awt.geom</code> package,
	 * then you should use a constructor that accepts a <code>Shape</code>
	 * argument instead of a <code>PathIterator</code> argument.
	 * 
	 * @param shape
	 *            the shape data to copy.
	 */
	 ...
	 *            the shape data to copy.
	 * @param transform
	 *            an optional transform to apply to the incoming shape.
	 */
	public BasicShape(Shape shape, AffineTransform transform) {
		...
		// to WIND_EVEN_ODD
				(shape instanceof CubicCurve2D) || (shape instanceof Line2D)
						|| (shape instanceof QuadCurve2D)
						|| (shape instanceof RectangularShape) // includes
																// arc2d,
																// ellipse2d,
																// rectangle2d,
																// and
	 ...
	 *            path data.
	 * @param ignoreWinding
	 *            if a path is WIND_NON_ZERO, then it normally will be
	 *            converted. However some shapes (rectangles, circles, etc)
	 *            don't need to be converted, so if you pass <code>true</code>
	 *            here then that conversion will be skipped. By default you
	 *            should pass <code>false</code> if you don't know the nature of
	 *            the <code>PathIterator</code> you're working with.
	 */
	 ...
	 *
	 * @param s
	 *            this string should be the result of a former call to: <br>
	 *            <code>myBasicShape.toString()</code>
	 */
	...
	/**
	 * This method should be called when this shape has undergone a
	 * transformation.
	 */
	...
	protected static boolean contains(BasicShapeSegment path, double x,
			double y, int windingRule, boolean includeEdges) {
	 ...
	 * <P>
	 * If shape B is added to shape A, then the resulting shape will contain
	 ...
	 * <P>
	 * If shape B is subtracted from shape A, then the resulting shape will
	 * contain only points in A that do not also lie in B.
	 * 
	 * @param shape
	 *            the shape to subtract. For best performance this should be a
	 *            BasicShape.
	 */
	...
	/**
	 * Intersects the argument shape with this shape. This is also referred to
	 * as "clipping".
	 * <P>
	 * If shape A is intersected with shape B, then the resulting shape will
	 * contain only points in both A and B.
	 * 
	 * @param shape
	 *            the shape to intersect with. For best performance this should
	 *            be a BasicShape.
	 */
	...
	static class Crossroad {
		...
		/**
		 * An arbitrary point behind the intersection Here "behind" the
		 * intersection means "as t decreases".
		 */
		Point2D prevP = new Point2D.Double();

		/**
		 * An arbitrary point in front of the intersection Here "in front of"
		 * the intersection means "as t increases".
		 */
	...
	private void performOperation(BasicShape shape, int op) {
		...
		try {
			...
			while (i != null) {
				...
				while (i.claimed == false) {
					if (direction == UNDEFINED) {
						/**
						 * Every intersection is like a crossroads. In the
						 * typical case, there are 4 directions in an
						 * intersection. We just came from 1, so there are 3
						 * choices to work with.
						 * 
						 * However. There are always 4 paths, but sometimes
						 * those paths will completely overlap. This means,
						 * visually, we're looking at 3 paths, maybe 2. I guess
						 * 1 path in an extreme case?
						 * 
						 * So in this method, "i" is the dot that is the
						 * intersection of these crossroads.
						 */

						current.init(i);
						other.init(i.complement);

						// ///////// Now pick which crossroad to follow, based
						// on the operation:

						if (op == ADD) {
							// always pick one that the other doesn't contain:
							// that way you're adding
							// as much as possible
							if (containsForAdd(i.s2, current.nextP) == false) {
								direction = FORWARD;
							} else if (containsForAdd(i.s2, current.prevP) == false) {
								direction = BACKWARD;
							} else if (containsForAdd(i.s1, other.nextP) == false
									&& lastDirection != BACKWARD) {
								i = i.complement;
								direction = FORWARD;
							} else if (containsForAdd(i.s1, other.prevP) == false
						...
						if (newPath && direction != AVOID) {
							...
							if (path == null) {
								path = new BasicShapeSegment(
										PathIterator.SEG_MOVETO, new double[] {
												p.getX(), p.getY() });
							} else {
								path = path.moveTo(p);
							}
					...
					/**
					 * This is the easy-ish part. We know where we are (at the
					 * Intersection "i"), and we know what direction we should
			...
			if (op == ADD || op == SUBTRACT) {
				path = addUntouchedPaths(path, shape, this,
						TYPE_UNTOUCHED_NOT_IN_OTHER);
			} else if (op == INTERSECT) {
				path = addUntouchedPaths(path, shape, this,
						TYPE_UNTOUCHED_ONLY_IN_OTHER);
			}
			if (shape != this) {
				if (op == ADD) {
					path = addUntouchedPaths(path, this, shape,
							TYPE_UNTOUCHED_NOT_IN_OTHER);
	 ...
	 * @param s
	 * @param other
	 *            if the path in s starts with a point in "other", then it is
	 *            NOT added
	 * @param dest
	 */
	private static BasicShapeSegment addUntouchedPaths(BasicShapeSegment path,
			BasicShape thisShape, BasicShape otherShape, int addType) {
			...
			if (t.type == PathIterator.SEG_MOVETO || t.next == null) {
				...
				if (foundIntersection == false && lastMoveTo != null) {
					...
					if (addType == TYPE_UNTOUCHED_ALL) {
						...
						addThis = sameShape
								|| (otherShape.contains(t2.data[0], t2.data[1]) == false);
					} else if (addType == TYPE_UNTOUCHED_ONLY_IN_OTHER) {
						addThis = sameShape
								|| (otherShape.contains(t2.data[0], t2.data[1]));
					}
					if (addThis) {
						if (path == null) {
							path = new BasicShapeSegment(
	 ...
	 * <P>
	 * This total is used in combination with the winding rule to determine when
	 * a point is inside a shape.
	 * 
	 * @param includeEdges
	 *            whether a point exactly on the edge of the shape counts.
	 */
	protected static int countCrossings(BasicShapeSegment t, double x,
			double y, boolean includeEdges) {
		t = t.getHead();
		...
		while (t != null) {
			if (t.type == PathIterator.SEG_MOVETO
					|| t.type == PathIterator.SEG_CLOSE) {
				// not an issue: movetos, well, they're nothing
				// and closes: when a BasicShape is constructed we close all the
				// subpaths
	...
	public boolean contains(double x, double y, double w, double h) {
		...
		/**
		 * We'll calculate this by looking at 3 criteria: 1. Do any path
		 * segments cross the edges of the rectangle? If so, we return false. 2.
		 * Do any path segments lie entirely inside the rectangle? If so, we
		 * return false. 3. Is at least 1 point inside this rectangle? If not,
		 * we return false.
		 */

		boolean contains = (x >= minX && y >= minY && (x + w) <= maxX && (y + h) <= maxY);
	...
	/**
	 * This checks 2 things: 1. Does this shape have any segments that lie
	 * completely inside the bounds provided? 2. Does this shape have any
	 * segments that cross the bounds provided?
	 * 
	 */
	protected boolean identifyCrossings(double x, double y, double w, double h) {
		...
		double by1 = maxY;
		while (t != null) {
			if (t.maxX > bx0 && t.maxY > by0 && t.minX < bx1 && t.minY < by1) {
				// only study a segment if we know it intersects our rectangle
				// of interest
				...
				if (t.type == PathIterator.SEG_LINETO) {
					...
					if (ay != 0) {
						// if they were equal, we'd have a horizontal line,
						// which
						// -- when you're looking for vertical intersections --
						// is not grounds to say this shape doesn't lie inside a
						// rectangle
	 ...
	 * This relies on an <code>IntersectionIdentifier</code> to find those
	 * intersections.
	 * <p>
	 * This replaces any previously existing intersection information in the two
	 * BasicShapes, so you cannot call this method on different threads
	 * simultaneously for the same shapes.
	 * 
	 * @param shape
	 *            the other shape to identify intersections with.
	...
	protected Intersection identifyIntersections(BasicShape shape) {
		...
		while (seg1 != null) {
			...
			if (seg1.type == PathIterator.SEG_LINETO) {
				...
				ay1 = -lastY1 + 3 * seg1.data[1] - 3 * seg1.data[3]
						+ seg1.data[5];
				by1 = 3 * lastY1 - 6 * seg1.data[1] + 3 * seg1.data[3];
				cy1 = -3 * lastY1 + 3 * seg1.data[1];
				dy1 = lastY1;

				ax1 = -lastX1 + 3 * seg1.data[0] - 3 * seg1.data[2]
						+ seg1.data[4];
			...
			if (seg1.type == PathIterator.SEG_LINETO
				...
				while (seg2 != null) {
					if (seg2.type == PathIterator.SEG_LINETO) {
						...
						ay2 = -lastY2 + 3 * seg2.data[1] - 3 * seg2.data[3]
								+ seg2.data[5];
						by2 = 3 * lastY2 - 6 * seg2.data[1] + 3 * seg2.data[3];
						cy2 = -3 * lastY2 + 3 * seg2.data[1];
						dy2 = lastY2;

						ax2 = -lastX2 + 3 * seg2.data[0] - 3 * seg2.data[2]
								+ seg2.data[4];
					...
					if (seg1.type == PathIterator.SEG_LINETO
						...
						hits = id.lineLine(ax1, bx1, ay1, by1, ax2, bx2, ay2,
								by2, dest, 0, Return.T1_T2);
					} else if (seg1.type == PathIterator.SEG_LINETO
							&& seg2.type == PathIterator.SEG_QUADTO) {
						hits = id.lineQuadratic(ax1, bx1, ay1, by1, ax2, bx2,
								cx2, ay2, by2, cy2, dest, 0, Return.T1_T2);
					} else if (seg1.type == PathIterator.SEG_LINETO
							&& seg2.type == PathIterator.SEG_CUBICTO) {
						hits = id.lineCubic(ax1, bx1, ay1, by1, ax2, bx2, cx2,
								dx2, ay2, by2, cy2, dy2, dest, 0, Return.T1_T2);
					} else if (seg1.type == PathIterator.SEG_QUADTO
							&& seg2.type == PathIterator.SEG_LINETO) {
						hits = id.lineQuadratic(ax2, bx2, ay2, by2, ax1, bx1,
								cx1, ay1, by1, cy1, dest, 0, Return.T1_T2);
						swap = true;
					} else if (seg1.type == PathIterator.SEG_QUADTO
							&& seg2.type == PathIterator.SEG_QUADTO) {
						hits = id.quadraticQuadratic(ax1, bx1, cx1, ay1, by1,
								cy1, ax2, bx2, cx2, ay2, by2, cy2, dest, 0,
								Return.T1_T2);
					} else if (seg1.type == PathIterator.SEG_QUADTO
							&& seg2.type == PathIterator.SEG_CUBICTO) {
						hits = id.quadraticCubic(ax1, bx1, cx1, ay1, by1, cy1,
								ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2, dest,
								0, Return.T1_T2);
					} else if (seg1.type == PathIterator.SEG_CUBICTO
							&& seg2.type == PathIterator.SEG_LINETO) {
						hits = id.lineCubic(ax2, bx2, ay2, by2, ax1, bx1, cx1,
								dx1, ay1, by1, cy1, dy1, dest, 0, Return.T1_T2);
						swap = true;
					} else if (seg1.type == PathIterator.SEG_CUBICTO
							&& seg2.type == PathIterator.SEG_QUADTO) {
						hits = id.quadraticCubic(ax2, bx2, cx2, ay2, by2, cy2,
								ax1, bx1, cx1, dx1, ay1, by1, cy1, dy1, dest,
								0, Return.T1_T2);
						swap = true;
					} else if (seg1.type == PathIterator.SEG_CUBICTO
							&& seg2.type == PathIterator.SEG_CUBICTO) {
						hits = id.cubicCubic(ax1, bx1, cx1, dx1, ay1, by1, cy1,
								cy1, ax2, bx2, cx2, dx2, ay2, by2, cy2, dy2,
								dest, 0, Return.T1_T2);
					}
					...
					if (hits > 0) {
						for (int a = 0; a < hits; a++) {
							Intersection i1, i2;
							if (swap == false) {
								i1 = seg1.addIntersection(seg2, dest[2 * a],
										dest[2 * a + 1]);
								i2 = seg2.addIntersection(seg1,
										dest[2 * a + 1], dest[2 * a]);
							} else {
								i1 = seg1.addIntersection(seg2,
										dest[2 * a + 1], dest[2 * a]);
								i2 = seg2.addIntersection(seg1, dest[2 * a],
										dest[2 * a + 1]);
							}
							i1.complement = i2;
							i2.complement = i1;
	 ...
	 * 
	 * @param at
	 *            an optional <code>AffineTransform</code> to apply to this
	 *            shape data
	 */
	...
	/**
	 * Creates a <code>FlatteningPathIterator</code> for the outline of this
	 * object.
	 * 
	 * @param at
	 *            an optional <code>AffineTransform</code> to apply to this
	 ...
	 * <P>
	 * Note that this <code>String</code> can later be passed as an argument to
	 * the constructor to re-create this shape, so this is a very efficient way
	 * to serialize this data.
	 */
	@Override
	public String toString() {
		StringBuffer sb = new StringBuffer(getPointCount() * 15); // guess how
																	// much
																	// space
																	// we'll
																	// want
	...
	/**
	 * When we are about to modify this object we need to synch on this object
	 * and call this method. This will make sure any iterators that still depend
	 * on this shape have a copy of their original data in tact, and they don't
	 * try to iterate over our modified data.
	 *
	 */
	protected void releaseIterators() {
		...
		while (dependentIterators.size() > 0) {
			WeakReference<BasicShapeIterator> r = dependentIterators
					.removeFirst();
	...
	class StubIdentifier extends IntersectionIdentifier {
		...
		@Override
		public int lineQuadratic(double ax1, double bx1, double ay1,
				double by1, double ax2, double bx2, double cx2, double ay2,
				double by2, double cy2, double[] dest, int offset,
				Return returnType) {
		...
		@Override
		public int quadraticCubic(double ax1, double bx1, double cx1,
				double ay1, double by1, double cy1, double ax2, double bx2,
		...
		public int cubicCubic(double ax1, double bx1, double cx1, double dx1,
				...
				double cy2, double dy2, double[] dest, int offset,
				Return returnType) {
			// TODO Auto-generated method stub
			return 0;
		}
