...
/**
 * This is a node in a double-linked list. Each node describes a path segment in
 * a shape.
 * 
 */
class BasicShapeSegment {
	 ...
	 * <li>For a MOVETO or LINETO segment, there will be 2 elements. <li>For a
	 * QUADTO segment, there will be 4 elements. <li>For a CUBICTO segment,
	 * there will be 6 elements. <li>For a CLOSE segment, there will be zero
	 * elements.</LI>
	 * 
	 */
	double[] data;
	...
	/**
	 * The list of intersections for this segment. This is created during an
	 * operation (such as add/intersect/etc.) and destroyed when that operation
	 * ends.
	 */
	Intersection i;
	...
	/**
	 * The minimum x-value this segment includes. Note this value is not
	 * accurate unless updateBounds() was recently called.
	 */
	double minX;

	/**
	 * The maximum x-value this segment includes. Note this value is not
	 * accurate unless updateBounds() was recently called.
	 */
	double maxX;

	/**
	 * The minimum y-value this segment includes. Note this value is not
	 * accurate unless updateBounds() was recently called.
	 */
	double minY;

	/**
	 * The maximum y-value this segment includes. Note this value is not
	 * accurate unless updateBounds() was recently called.
	 */
	double maxY;

	/**
	 * Appends a segment to this node. As of this writing: it is assumed this is
	 * only used to append nodes to the tail of this list. An exception is
	 * thrown if you try to insert an element arbitrarily in the list.
	 * 
	 * @param segment
	 * @return the new tail.
	 */
	protected BasicShapeSegment append(BasicShapeSegment segment) {
		...
		// it's OK to change this in the future, so long as there's a good
		// reason.
		if (next != null)
			throw new RuntimeException(
					"illegal attempt to insert node with the append method");
	...
	/**
	 * Appends part of the path to this node. It is assumed this node is the
	 * tail of this the list.
	 * 
	 * <p>
	 * Note the starting time may be 1, and the ending time may be 0, indicating
	 * that the argument segment should be written backwards.
	 * 
	 * @param newSegment
	 *            the shape segment to append to this node.
	 * @param t0
	 *            the starting time. This will be within [0,1].
	 * @param t1
	 *            the ending time. This will be within [0,1].
	 * @return the new tail of this shape
	 */
	protected BasicShapeSegment append(BasicShapeSegment newSegment, double t0,
			double t1) {
		double[] array = new double[8];
		int i = newSegment.subdivide(t0, t1, array);
		if (i == 2) {
			return append(new BasicShapeSegment(PathIterator.SEG_LINETO,
					new double[] { array[2], array[3] }));
		} else if (i == 3) {
			return append(new BasicShapeSegment(PathIterator.SEG_QUADTO,
					new double[] { array[2], array[3], array[4], array[5] }));
		} else if (i == 4) {
			return append(new BasicShapeSegment(PathIterator.SEG_CUBICTO,
					new double[] { array[2], array[3], array[4], array[5],
							array[6], array[7] }));
		} else {
	 ...
	 * 
	 * This in no way modifies the otherSegment argument; that should be done
	 * separately. And once two Intersections are created, they should identify
	 * each other as complements.
	 * 
	 * @param otherSegment
	 * @param myTime
	 * @param otherTime
	 ...
	 */
	protected Intersection addIntersection(BasicShapeSegment otherSegment,
			double myTime, double otherTime) {
	...
	protected BasicShapeSegment moveTo(double x, double y) {
		BasicShapeSegment newGuy = new BasicShapeSegment(
				PathIterator.SEG_MOVETO, new double[] { x, y });
		append(newGuy);
		return newGuy;
	}
	...
	protected BasicShapeSegment lineTo(double x, double y) {
		BasicShapeSegment newGuy = new BasicShapeSegment(
				PathIterator.SEG_LINETO, new double[] { x, y });
		append(newGuy);
		return newGuy;
	}
	...
	protected BasicShapeSegment quadTo(double cx, double cy, double x, double y) {
		BasicShapeSegment newGuy = new BasicShapeSegment(
				PathIterator.SEG_QUADTO, new double[] { cx, cy, x, y });
		append(newGuy);
		return newGuy;
	}

	protected BasicShapeSegment cubicTo(double cx1, double cy1, double cx2,
			double cy2, double x, double y) {
		BasicShapeSegment newGuy = new BasicShapeSegment(
				PathIterator.SEG_CUBICTO, new double[] { cx1, cy1, cx2, cy2, x,
						y });
		append(newGuy);
		return newGuy;
	}
	...
	/**
	 * If this node is not a SEG_CLOSE node, then this will append a SEG_CLOSE
	 * node.
	 * <P>
	 * This may also append a SEG_LINETO node, if necessary, before the
	 * SEG_CLOSE to bring the subpath back to the desired point.
	 * 
	 * @param x
	 *            the x-coordinate the subpath should end at.
	 * @param y
	 *            the y-coordinate the subpath should end at.
	 * @return the new tail.
	 */
	private BasicShapeSegment closeTo(double x, double y) {
		...
		if (!(Math.abs(data[data.length - 2] - x) < .00001 && Math
				.abs(data[data.length - 1] - y) < .00001)) {
		...
		current = current.append(new BasicShapeSegment(PathIterator.SEG_CLOSE,
				EMPTY_DOUBLE_ARRAY));
	...
	/**
	 * Creates a series of segments representing the path presented in the
	 * argument.
	 * <P>
	 * The segments will represent a safe, closed shape, regardless of what the
	 * argument is.
	 * <P>
	 * This may return <code>null</code> if the argument is empty.
	 * 
	 * @param i
	 *            a path.
		...
		while (i.isDone() == false) {
			...
			if (type == PathIterator.SEG_CLOSE
				...
				if (tail != null && tail.type != PathIterator.SEG_CLOSE) {
					// if the last node were a SEG_CLOSE, then data.length would
					// be zero.
					// So we didn't close the last path. Paths always must be
					// closed.
				...
				// type =
				// SimplifiedPathIterator.simplify(type,lastX,lastY,scratchArray);
				...
				if (type == PathIterator.SEG_LINETO) {
					...
					array = new double[] { scratchArray[0], scratchArray[1],
							scratchArray[2], scratchArray[3] };
				} else {
					array = new double[] { scratchArray[0], scratchArray[1],
							scratchArray[2], scratchArray[3], scratchArray[4],
							scratchArray[5] };
				}
				...
				if (head == null) {
					...
					throw new RuntimeException(
							"Illegal segment data without a moveTo instruction.");
				}
	 ...
	 *            the segment type
	 * @param dataArray
	 *            the array to use as the data. Note this array is not cloned.
	 */
	...
	/**
	 * Subdivides this into a segment of the same type that maps t0 to 0 and t1
	 * to 1.
	 * 
	...
	/**
	 * Iterates one step forward in this subpath. This will not return the
	 * SEG_CLOSE segment, it will skip to the next segment.
	 */
	...
	/**
	 * Iterates one step backward in this subpath. This will not return the
	 * SEG_CLOSE segment, it will skip to the previous segment.
	 */
	...
	/**
	 * Stores the (x,y) value of this segment at time t. Note this throws an
	 * exception if this segment is a MOVETO or CLOSE segment.
	 * 
	 * @param p
	 *            the destination to store the point in
	 * @param t
	 *            the time
	 */
	protected Point2D getPoint(Point2D p, double t) {
		...
		if (type == PathIterator.SEG_LINETO) {
			...
			p.setLocation(((ax * t + bx) * t + cx) * t + dx,
					((ay * t + by) * t + cy) * t + dy);
	...
	/**
	 * Returns the x-value for this segment at the specified time. Note this
	 * throws an exception if this segment is a MOVETO or CLOSE segment.
	 * 
	 */
	...
	/**
	 * Returns the y-value for this segment at the specified time. Note this
	 * throws an exception if this segment is a MOVETO or CLOSE segment.
	 * 
	 */
	...
	protected double getAngle(double t) {
		...
		if (type == PathIterator.SEG_LINETO) {
			...
			return Math.atan2(3 * ay * t * t + 2 * by * t + cy, 3 * ax * t * t
					+ 2 * bx * t + cx);
		} else if (type == PathIterator.SEG_CLOSE) {
			throw new RuntimeException("unexpect close");
		} else if (type == PathIterator.SEG_MOVETO) {
	 ...
	 * <P>
	 * This does NOT sort the times, eliminate duplicate times, or filter for
	 * times exclusively between [0,1]. This just list the times.
	 * 
	 * @param x
	 * @param y
	 * @param dest
	 *            the array to store the t-values in
	 * @return the number of t-values found
	 */
	protected int getTimes(double x, double y, double[] dest) {
		if (type == PathIterator.SEG_MOVETO) {
			if (BasicShape.near(x, data[0], .0001)
					&& BasicShape.near(y, data[1], .0001)) {
				dest[0] = 1;
				return 1;
			}
			return 0;
			...
			double ax = -prev.data[prev.data.length - 2] + 3 * data[0] - 3
					* data[2] + data[4];
			double bx = 3 * prev.data[prev.data.length - 2] - 6 * data[0] + 3
					* data[2];
			double cx = -3 * prev.data[prev.data.length - 2] + 3 * data[0];
			double dx = prev.data[prev.data.length - 2];

			double ay = -prev.data[prev.data.length - 1] + 3 * data[1] - 3
					* data[3] + data[5];
			double by = 3 * prev.data[prev.data.length - 1] - 6 * data[1] + 3
					* data[3];
			...
			for (int a = 0; a < i; a++) {
				double myX = ((ax * results[a] + bx) * results[a] + cx)
						* results[a] + dx;
			...
			for (int a = 0; a < i; a++) {
				double myY = ((ay * results[a] + by) * results[a] + cy)
						* results[a] + dy;
	...
	/**
	 * Finds a value t between [0,1] at which this segment contains the point
	 * (x,y). Returns -1 if no such time is found.
	 * 
	 * @param x
	 *            the x-coordinate
	 * @param y
	...
	/**
	 * A string representation for this segment only. Intended only for
	 * debugging.
	 */
	@Override
	...
	/**
	 * A string representation for this segment. Intended only for debugging.
	 * 
	 * @param allSubsequentNodes
	 *            if <code>true</code>, then all the future nodes will also be
	 *            listed in this string.
	 */
	...
	/**
	 * Clones this segment and all its subsequent segments, but not all its
	 * previous segments.
	 */
	@Override
	public Object clone() {
		...
		while (current != null) {
			...
			BasicShapeSegment clone = new BasicShapeSegment(current.type,
					cloneData);
	...
	/**
	 * This updates the min/max values cached in this segment, and all future
	 * (but not previous) segments.
	 * <P>
	 * This segment itself has no concept of when these values are accurate or
	 * need to be updated.
	 */
