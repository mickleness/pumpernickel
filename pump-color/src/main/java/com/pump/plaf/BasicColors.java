...
public class BasicColors extends ColorSet {
	static float[] popularHues = new float[] { 0, 1.0f / 12.0f, 1.0f / 6.0f,
			1.0f / 3.0f, .5f, 2.0f / 3.0f, 5.0f / 6.0f };
	...
	public BasicColors(boolean grid) {
		this(grid, 6, BasicColors.popularHues, popularHues.length, new int[][] {
				grays, browns });
	}

	public BasicColors(boolean grid, int rowCount, float[] hues,
			int hueColumns, int[][] columns) {
	...
	public int getRGB(float xFraction, float yFraction) {
		...
		if (xFraction < cutOff) {
			...
			float p = (xFraction - ((float) index) / ((float) hues.length))
					* hues.length;
			...
			return Color.HSBtoRGB(tweenHues(hues[index], hues[nextIndex], p),
			...
			if (fraction > 1)
				fraction = 1;
			return tweenRGB(specialColumns[index][0], specialColumns[index][1],
					fraction);
		}
	}
	...
	public Point2D getRelativePoint(int rgb) {
		...
		for (int index = 0; index < hues.length; index++) {
			...
			if (minHue <= hue && hue <= maxHue) {
				...
				xFraction = ((float) index) / ((float) hues.length) + t
						/ (hues.length);
				xFraction = (xFraction + .5f / (columns) + 1) % 1;

				float cutOff = 1 - ((float) specialColumns.length)
						/ ((float) columns);
				xFraction = xFraction * cutOff;
			}
		}
		...
		for (int a = 0; a < columnRGBs.length; a++) {
			...
			double[] quadratic = new double[] {
					r1 * r1 - 2 * r1 * r2 + r2 * r2 + g1 * g1 - 2 * g1 * g2
							+ g2 * g2 + b1 * b1 - 2 * b1 * b2 + b2 * b2,
					2 * r * r1 - 2 * r * r2 - 2 * r1 * r1 + 2 * r1 * r2 + 2 * g
							* g1 - 2 * g * g2 - 2 * g1 * g1 + 2 * g1 * g2 + 2
							* b * b1 - 2 * b * b2 - 2 * b1 * b1 + 2 * b1 * b2,
					r * r - 2 * r * r1 + r1 * r1 + g * g - 2 * g * g1 + g1 * g1
							+ r * r - 2 * b * b1 + b1 * b1 };
			...
			if (minT >= -0.000001 && minT <= 1.000001) {
				...
				if (error <= minError) {
					minError = error;
					xFraction = 1
							- ((float) (specialColumns.length - a + .5 - 1))
							/ (columns);
					float dy = .5f / (rows);
