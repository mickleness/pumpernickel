...
public class HSBColorPaletteUI extends ScrollableColorPaletteUI {
	...
	/**
	 * The client property that is associated with how the palette is vertically
	 * padded. This refers to whether the zeroeth and/or the last vertical cell
	 * represent a complete saturation/desaturation, or whether they are
	 * slightly offset.
	 */
	...
	protected ColorSet getColorSet(ColorPalette cp, boolean grid) {
		...
		return new HBColors(hues, grid, rows, columns, saturation, alpha,
				(String) cp.getClientProperty(PALETTE_PADDING_PROPERTY));
	}
	...
	protected void updateColor(ColorPalette cp) {
		...
		HBColors colorSet = new HBColors(hues, true, rows, columns, hsb[1], 1,
				(String) cp.getClientProperty(PALETTE_PADDING_PROPERTY));
	...
	public void installUI(JComponent c) {
		...
		cp.addPropertyChangeListener(PALETTE_PADDING_PROPERTY,
	...
	public void uninstallUI(JComponent c) {
		...
		ColorPalette cp = (ColorPalette) c;
		cp.removePropertyChangeListener(PALETTE_PADDING_PROPERTY,
				repaintPropertyListener);
	}
	...
	static class HBColors extends ColorSet {
		 ...
		 */
		public HBColors(HueDistribution hueDistribution, boolean grid,
