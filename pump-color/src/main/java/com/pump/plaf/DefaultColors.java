...
public class DefaultColors extends ColorSet {
	...
	public Point2D getRelativePoint(int rgb) {
		...
		/**
		 * This palette is really a combination of 3 palettes. Which one is
		 * closest to the value we're looking for?
		 */

		int gray = (red + green + blue) / 3;
		int grayError = (red - gray) * (red - gray) + (green - gray)
				* (green - gray) + (blue - gray) * (blue - gray);
		...
		int lightError = (red - lightRed) * (red - lightRed)
		...
		int darkError = (red - darkRed) * (red - darkRed) + (green - darkGreen)
				* (green - darkGreen) + (blue - darkBlue) * (blue - darkBlue);
		...
		if (grayError <= lightError && grayError <= darkError) {
			...
			if (red == 255 && green == 255 && blue == 255) {
				...
				yFraction = whiteLimit + (1 - (gray) / 255f)
