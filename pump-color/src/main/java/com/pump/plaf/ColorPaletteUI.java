...
public abstract class ColorPaletteUI extends ComponentUI {
	...
	/**
	 * The client property for the relative point. This maps to a Point2D
	 * indicating where the selected color is physically located. This is
	 * especially useful in the cases where a color is represented in several
	 * possible coordinates (such as the color black if the bottom row of a HSB
	 * spectrum is 100% black).
	 */
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		return new Dimension(set.columns * cellWidth + insets.left
				+ insets.right, set.rows * cellHeight + insets.top
				+ insets.bottom);
	}
	...
	public void installUI(JComponent c) {
		...
		cp.addPropertyChangeListener(ColorSet.PALETTE_STYLE_PROPERTY,
				repaintPropertyListener);
		cp.addPropertyChangeListener(ColorSet.PALETTE_CELL_STYLE_PROPERTY,
				repaintPropertyListener);
		cp.addPropertyChangeListener(RELATIVE_POINT_PROPERTY,
				repaintPropertyListener);
	...
	public void uninstallUI(JComponent c) {
		...
		c.removePropertyChangeListener(ColorSet.PALETTE_STYLE_PROPERTY,
				repaintPropertyListener);
		c.removePropertyChangeListener(ColorSet.PALETTE_CELL_STYLE_PROPERTY,
				repaintPropertyListener);
		c.removePropertyChangeListener(RELATIVE_POINT_PROPERTY,
				repaintPropertyListener);
	...
	protected void processKeyEvent(KeyEvent keyEvent, int dx, int dy) {
		...
		if (colors.grid) {
			...
			if (column < 0)
				column = 0;
			if (column >= colors.columns)
				column = colors.columns - 1;
		...
		Color color = new Color(getColorSet(cp).getRGB((float) newX,
				(float) newY), true);
		...
		cp.putClientProperty(RELATIVE_POINT_PROPERTY, new Point2D.Double(newX,
				newY));
		keyEvent.consume();
	}
	...
	protected void processMouseEvent(MouseEvent e, float xFraction,
		...
		if (colors.grid) {
			...
			p.setLocation((column + .5f) / (colors.columns), (row + .5f)
					/ (colors.rows));
		}
		p.setLocation(Math.max(0, Math.min(p.getX(), 1)),
				Math.max(0, Math.min(p.getY(), 1)));
		Color color = new Color(getColorSet(cp).getRGB((float) p.getX(),
				(float) p.getY()), true);
		MutableInteger adjusting = getAdjustingLock(cp);
		adjusting.value++;
		try {
	...
	/**
	 * Resets the UI to feature the color returned by <code>cp.getColor()</code>
	 * , if possible.
	 */
	...
	public void paint(Graphics g0, JComponent c) {
		if (c.getBorder() != null) {
			...
			g0.fillRect(borderInsets.left, borderInsets.top, c.getWidth()
		...
		if (isRelativePointValid(cp)) {
			...
			g.draw(e);
			e.setFrame(p.getX() * r.width - 3 + r.x, p.getY() * r.height - 3
					+ r.y, 6, 6);
		 ...
		 * System.out.println(cp.getColor()); g.setColor(Color.black);
		 * g.fillRect( (int)(zz.getX()*r.width-2)+r.x,
		 * (int)(zz.getY()*r.height-2)+r.y, 4, 4);
		 */
	}
	...
	protected static int getRGBDistanceSquared(int rgb1, int rgb2) {
		...
		int g2 = (rgb2 >> 8) & 0xff;
		int b2 = (rgb2 >> 0) & 0xff;
		int distanceSquared = (a1 - a2) * (a1 - a2) + (r1 - r2) * (r1 - r2)
