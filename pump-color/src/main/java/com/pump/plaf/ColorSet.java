 ...
 * <p>
 * Here are possible customizations the {@link #PALETTE_CELL_STYLE_PROPERTY} and
 * {@link #PALETTE_STYLE_PROPERTY} allow:
 * <p>
 * <table summary="Sample Color Set Configurations">
 * <tr>
 * <td></td>
 * <td>Default Cell</td>
 * <td>Shadow Cell</td>
 ...
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_default_default.png"
 * alt="Default Palette Style, Default Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_default_shadow.png"
 * alt="Default Palette Style, Shadow Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_default_scribble.png"
 * alt="Default Palette Style, Scribble Cell Style"></td>
 * </tr>
 * <tr>
 * <td>Gradient Palette</td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_gradient_default.png"
 * alt="Gradient Palette Style, Default Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_gradient_shadow.png"
 * alt="Gradient Palette Style, Shadow Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_gradient_scribble.png"
 * alt="Gradient Palette Style, Scribble Cell Style"></td>
 * </tr>
 * <tr>
 * <td>Streaks Palette</td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_streaks_default.png"
 * alt="Streak Palette Style, Default Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_streaks_shadow.png"
 * alt="Streak Palette Style, Shadow Cell Style"></td>
 * <td><img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/ColorPaletteDemo/style_streaks_scribble.png"
 * alt="Streak Palette Style, Scribble Cell Style"></td>
 * </tr>
 * </table>
 * 
 * TODO: rename to distinguish from
 * <code>com.pump.pixel.image.quantize.ColorSet</code>
 */
public abstract class ColorSet {
	...
	/**
	 * The client property name that maps to one of the PALETTE_CELL_STYLE
	 * constants.
	 * 
	 ...
	 * 
	 * @param grid
	 *            whether this is rendered with a grid or not.
	 */
	 ...
	 * 
	 * @param size
	 *            the dimensions of the image to paint
	 * @param applyBorder
	 *            whether a simple border should be applied to the new image
	...
	public BufferedImage getImage(int w, int h, JComponent jc) {
		...
		String style = jc == null ? null : (String) jc
				.getClientProperty(PALETTE_STYLE_PROPERTY);
		if (style == null)
			style = PALETTE_STYLE_DEFAULT;

		String cellStyle = jc == null ? null : (String) jc
				.getClientProperty(PALETTE_CELL_STYLE_PROPERTY);
		if (cellStyle == null)
		...
		if (image != null
				...
				&& cellStyle.equals(image
						.getProperty(PALETTE_CELL_STYLE_PROPERTY))) {
			return image;
		}

		MutableBufferedImage bi = new MutableBufferedImage(w, h,
	 ...
	 */
	/*
	 * private void findDuplicates(BufferedImage bi) { int w = bi.getWidth();
	 * int h = bi.getHeight(); int[] data = new int[w*h];
	 * bi.getRaster().getDataElements(0, 0, w, h, data); int size = w*h; for(int
	 * a = 0; a<size; a++) { int argb =
	 * (int)(0xff000000+Math.random()*0xffffff); for(int b = a+1; b<size; b++) {
	 * if(data[b]==data[a]) { data[b] = argb; data[a] = argb; } } }
	 ...
	 * @param bi
	 * @param style
	 *            the palette style. This should be one of the PALETTE_STYLE
	 *            constants, and when defined by a JComponent: it should be the
	 *            value of the PALETTE_STYLE_PROPERTY key.
	 * @param cellStyle
	 *            the palette cell style. This should be one of the
	 *            PALETTE_CELL_STYLE constants, and when defined by a
	 *            JComponent: it should be the value of the
	 *            PALETTE_CELL_STYLE_PROPERTY key.
	 */
	public void paint(BufferedImage bi, String style, String cellStyle) {
		if (!(bi.getType() == BufferedImage.TYPE_INT_ARGB || bi.getType() == BufferedImage.TYPE_INT_RGB))
			throw new IllegalArgumentException(
					"an image of type RGB or ARGB is required");
		...
		if (grid) {
			...
			if (cellStyle != null
					&& cellStyle.equals(PALETTE_CELL_STYLE_SHADOW)) {
				paintGridShadowHighlights(bi);
			} else if (cellStyle != null
					&& cellStyle.equals(PALETTE_CELL_STYLE_SCRIBBLE)) {
	...
	protected void paintGridShadowHighlights(BufferedImage bi) {
		...
		Color light1 = new Color(255, 255, 255,
				(int) (10 * getHighlightAlpha()));
		Color light2 = new Color(255, 255, 255,
				(int) (20 * getHighlightAlpha()));
		...
		for (int col = 0; col < columns; col++) {
			...
			for (int x = x0; x <= x1; x++) {
				...
				if (x == x0) {
					...
					while (y < h) {
						float yFraction = (y) / ((float) h - 1);
						float xFraction2 = Math.max(0,
								xFraction + (random.nextFloat() * xVariance));
						...
						y += streakHeight;
						streakHeight = Math.max(random.nextInt(streakMax),
								streakMin);
					}
					if (lastData != null) {
						for (y = 0; y < h; y++) {
	...
	public Point getPoint(int rgb, int width, int height) {
		...
		Point p2 = new Point((int) (p.getX() * width),
				(int) (p.getY() * height));
		return p2;
	}
