...
public class ImageResizerDeleteMe {
	public static void main(String[] args) {
		...
		while(iter.hasNext()) {
			...
			try {
				...
				System.err.println("Couldn't validate (or delete): "+f.getAbsolutePath());
				e.printStackTrace();
			}
		}
		...
		while(iter.hasNext()) {
			...
			try {
				...
				if(bi.getWidth()>bi.getHeight()) {
					bi2 = Scaling.scaleProportionally(bi, new Dimension(1024, 768));
				} else {
					bi2 = Scaling.scaleProportionally(bi, new Dimension(768, 1024));
				}
				if(bi2.getWidth()!=bi.getWidth() || bi2.getHeight()!=bi.getHeight()) {
					...
					if(f.getAbsolutePath().equals(dest.getAbsolutePath())) {
						System.out.println("Resaved "+f.getAbsolutePath()+" ("+localSaved+")");
					} else {
						System.out.println("Converted "+f.getAbsolutePath() + " to " + dest.getName()+" ("+localSaved+")");
					}
					saved += localSaved;
				}
			} catch(Exception e) {
	...
	protected static boolean isFileMatch(Collection<File> candidateFiles, File file) throws IOException {
