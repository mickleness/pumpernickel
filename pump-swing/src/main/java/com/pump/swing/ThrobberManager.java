 ...
 * <p>
 * Call {@link #createToken()} to activate all possible throbbers,
 * and then after some time call {@link #returnToken(Object)} to return
 * the token.
 * <p>
 * Your usage should resemble:
 * <pre> Thread myThread = new Thread() {
 *   public void run() {
 ...
 * };</pre>
 * <p>
 * Any JThrobbers this object creates will automatically toggle their visibility based on
 * whether any tokens have been checked out.
 * 
 */
public class ThrobberManager {
	 ...
	 * <p>
	 * This is a convenience method for creating an empty constructor and then calling
	 * {@link #join(JobManager)}.
	 */
	...
	/**
	 * This combines this ThrobberManager with the JobManager provided.
	 * When the JobManager is active: this ThrobberManager will also be active.
	 * You may continue to independently use {@link #createToken()} to further add
	 * activity to this manager. (For that matter, you may also join this
	 * manager to multiple JobManagers if you want to.)
	 * 
	 * @param jobManager the JobManager to listen to.
	 */
	...
	/** Create a new JThrobber that will automatically toggle its
	 * visibility based on whether any entity has borrowed tokens
	...
	/** Return true if any threads have borrowed tokens against this
	 * manager.
	 */
	...
	/**
	 * Add a ChangeListener to be notified when the number of borrowed
	 * tokens changes. This is only notified when the first token is borrowed
	 * or the last token is returned. (That is: this only distinguishes between "active"
	 * and "inactive". It doesn't notify listeners if a 2nd token is borrowed, because
	 * then it only changes from "active" to "active".)
	 * 
	 * @param cl
	 */
	 ...
	 * @param token an object previously created via {@link #createToken()}.
	 */
