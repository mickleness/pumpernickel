...
/**
 * As you mouse over components inside this panel, this panel will repaint
 * itself to show positioning information.
 * <P>
 * This in no way manipulates the components or their layout: it only changes
...
public class GridBagDebugPanel extends JPanel {
	...
	MouseInputAdapter mouseListener = new MouseInputAdapter() {
		...
		public void mouseMoved(MouseEvent e) {
			...
			p = SwingUtilities.convertPoint(e.getComponent(), p,
					GridBagDebugPanel.this);
			for (int a = 0; a < getComponentCount(); a++) {
				Component child = getComponent(a);
				Point p2 = SwingUtilities.convertPoint(GridBagDebugPanel.this,
						p, child);
				...
				if (inside && indicatedComponents.contains(child) == false) {
					...
					repaint();
						...
						&& indicatedComponents.contains(child) == true) {
	...
	public static void setOpacity(float f) {
		...
		int alpha = (int) (f * 255);
		lightRed = new Color(lightRed.getRed(), lightRed.getGreen(),
				lightRed.getBlue(), alpha);
		lightGreen = new Color(lightGreen.getRed(), lightGreen.getGreen(),
				lightGreen.getBlue(), alpha);
		lightBlue = new Color(lightBlue.getRed(), lightBlue.getGreen(),
				lightBlue.getBlue(), alpha);
	}
	...
	static BasicStroke dottedStroke1 = new BasicStroke(1, BasicStroke.CAP_BUTT,
			BasicStroke.JOIN_BEVEL, 10, new float[] { 2, 2 }, 0);
	static BasicStroke dottedStroke2 = new BasicStroke(1, BasicStroke.CAP_BUTT,
			BasicStroke.JOIN_BEVEL, 10, new float[] { 2, 2 }, 1);
