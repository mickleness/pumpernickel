 ...
 *
 * $Date: 2016-01-05 17:20:17 -0500 (Tue, 05 Jan 2016) $
 *
		...
		try {
			...
			if(rect0.y+rect0.height==rect1.y+rect1.height) {
				...
				drawLines : while(true) {
					...
					if(p1<=pos) {
						drawLine(g, lastPos, p1, startX, rect1.x + rect1.width, currentY, squiggle);
						break drawLines;
					} else {
						r = c.modelToView( Math.min(pos, p1) );
						drawLine(g, lastPos, pos, startX, r.x + r.width, currentY, squiggle);
