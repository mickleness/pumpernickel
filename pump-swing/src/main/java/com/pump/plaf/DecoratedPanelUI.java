...
/**
 * This <code>PanelUI</code> offers several attributes to provide more modern
 * and sophisticated panels.
 * <p>
 * The attributes are broken up into two groups:
 * <ul>
 * <li>Border Attributes: these are applied to the border, which can be applied
 * separately as a <code>javax.swing.Border</code>.</li>
 * <li>Panel Attributes: these are applied to the panel as a whole, which means
 * they affect the middle of the panel as well as the border.</li>
 * </ul>
 * <p>
 * The {@link Renderer} class manages painting all attributes. Both the
 * {@link Border} and the <code>DecoratedPanelUI</code> defer to the same
 * <code>Renderer</code> to paint everything. (But both entities clip the
 * Graphics2D destination strategically to only paint the appropriate portions.)
 * <p>
 * Here are examples of some of the harder-to-describe attributes. (These
 * samples may not show each feature in the most compelling/flattering
 * presentation, but they should help demonstrate what different attributes do.)
 * <p>
 * <table summary="Sample DecoratedPanel Configurations" cellpadding="20">
 * <tr>
 * <td>
 * Glaze:
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/glaze-demo.gif"
 * alt="Glaze Demo"></td>
 * <td>
 * Bevel:
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/bevel-demo.gif"
 * alt="Bevel Demo"></td>
 * <td>
 * Corners:
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/corner-demo.gif"
 * alt="Corner Demo"></td>
 * </tr>
 * <tr>
 * <td>
 ...
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/scribble-demo.gif"
 * alt="Scribble Demo"></td>
 * <td>
 * Shadow:
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/shadow-demo.gif"
 * alt="Shadow Demo"></td>
 * </tr>
 * </table>
 * 
 ...
 * @see <a
 *      href="http://javagraphics.blogspot.com/2014/03/panels-improved-panelui.html">Panels:
 *      an Improved UI</a>
 */
@Blurb(title = "Panels: an Improved PanelUI", releaseDate = "March 2014", summary = "This explores a new <code>PanelUI</code> to spruce up panels. This achieves a variety "
		...
		+ "a separate <code>javax.swing.Border</code> that can be used independently of this UI.", article = "http://javagraphics.blogspot.com/2014/03/panels-improved-panelui.html", imageName = "DecoratedPanelUI.png")
public class DecoratedPanelUI extends PanelUI {
	 ...
	 * <p>
	 * <img src=
	 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/createDarkRecessedUI.png"
	 * alt="Dark Recessed Demo">
	 */
	public static DecoratedPanelUI createDarkRecessedUI() {
		...
		ui.setColors(blendColors(UIManager.getColor("Panel.background"),
				Color.black, .05f));
		return ui;
	}
	 ...
	 * <p>
	 * <img src=
	 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/createMinimalRoundedUI.png"
	 * alt="Minimal Rounded Demo">
	 */
	 ...
	 * <p>
	 * <img src=
	 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/createSubtleShadowUI.png"
	 * alt="Subtle Shadow Demo">
	 */
	...
	/**
	 * Create a <code>DecoratedPanelUI</code> with a scribble border (resembling
	 * torn paper) and a light shadow/border.
	 * <p>
	 * <img src=
	 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/createSubtleScribbleUI.png"
	 * alt="Subtle Scribble Demo">
	 */
	 ...
	 * <p>
	 * <img src=
	 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/decorated-panel/createPlasticUI.png"
	 * alt="Plastic Demo">
	 */
	...
	/**
	 * This is used by both the {@link Border} and the {@link DecoratedPanelUI}
	 * to render all the attributes.
	 * <p>
	 * You can override some methods to provide unique looks. For example by
	 * override the {@link #paintBody(Graphics2D, Component, Map)} method you
	 * can fill the body with a texture paint to resemble brushed metal (which
	 * makes the attributes {@link DecoratedPanelUI#COLOR_TOP} and
	 * {@link DecoratedPanelUI#COLOR_BOTTOM} irrelevant). If customizing
	 * renderers via subclasses becomes a common pattern then this class should
	 * be refactored to somehow take that into account.
	 */
	public static class Renderer {
		...
		/**
		 * Redefine the GeneralPath argument provided with the new border/body
		 * outline of this UI.
		 * 
		 * @param c
		 *            the component being painted.
		 * @param map
		 *            the attributes. At any given time: some attributes might
		 *            be missing.
		 * @param body
		 *            the path this method redefines.
		 * @param extraPadding
		 *            a positive integer that expands the size of this shape.
		 */
		protected void updateBodyShape(Component c, Map<String, Object> map,
				GeneralPath body, int extraPadding) {
			...
			int shadowLayers = DecoratedPanelUI.Border.DROP_SHADOW_LAYER_COUNT
					.get(map, 0);
			int scribblePadding = (int) (DecoratedPanelUI.Border.SCRIBBLE_SIZE
					.get(map, 0f) + .5);
			Insets insets = max(new Insets(scribblePadding, scribblePadding,
					scribblePadding, scribblePadding), new Insets(shadowLayers,
					shadowLayers, shadowLayers, shadowLayers));
			...
			float cornerSizeLowerLeft = Border.CORNER_SIZE_LOWER_LEFT.get(map,
					0f);
			float cornerSizeLowerRight = Border.CORNER_SIZE_LOWER_RIGHT.get(
					map, 0f);
			float cornerSizeUpperLeft = Border.CORNER_SIZE_UPPER_LEFT.get(map,
					0f);
			float cornerSizeUpperRight = Border.CORNER_SIZE_UPPER_RIGHT.get(
					map, 0f);
			...
			if (cornerSizeUpperRight > 0) {
				...
				b.curveTo(w + cornerSizeUpperRight * (z - 1) - k + j, k - j, w
						- k + j, cornerSizeUpperRight * (1 - z) + k - j, w - k
						+ j, cornerSizeUpperRight + k - j);
			} else {
				b.lineTo(w - k + j, k - j);
			}
			...
			if (cornerSizeLowerRight > 0) {
				...
				b.curveTo(w - k + j,
						h + cornerSizeLowerRight * (z - 1) - k + j, w
								+ cornerSizeLowerRight * (z - 1) - k + j, h - k
								+ j, w - cornerSizeLowerRight - k + j, h - k
								+ j);
			} else {
			...
			if (cornerSizeLowerLeft > 0) {
				...
				b.curveTo(cornerSizeLowerLeft * (1 - z) + k - j, h - k + j, k
						- j, h + cornerSizeLowerLeft * (z - 1) - k + j, k - j,
						h - cornerSizeLowerLeft - k + j);
			} else {
				b.lineTo(k - j, h - k + j);
			}
			...
			b.closePath();
			b.transform(AffineTransform.getTranslateInstance(insets.left,
					insets.top));
			...
			if (scribbleSize != 0) {
				Scribbler.create(b, scribbleSize, scribbleSize, 0,
						new GeneralPathWriter(body));
			} else {
				body.append(b, false);
			}
		}
		...
		/**
		 * Redefine the GeneralPath argument provided with the new glaze outline
		 * of this UI.
		 * 
		 * @param c
		 *            the component being painted.
		 ...
		 *            the path this method redefines.
		 */
		protected void updateGlazeShape(Component c,
				Map<String, Object> attributes, GeneralPath glaze) {
			int w = c.getWidth();
			int h = c.getHeight();
			...
			glaze.curveTo(0, (1 - z) * (h * .8f) + z * h * .6f, (1 - z) * w
					* 1f / 8f + z * w * 1f / 8f, (1 - z) * (h * .7f) + z * h
					* .5f, (1 - z) * w * 1f / 2f + z * w * 1f / 2f, (1 - z)
					* (h * .7f) + z * h * .5f);
			glaze.curveTo((1 - z) * w * 7f / 8f + z * w * 7f / 8f, (1 - z) * h
					* .7f + z * h * .5f, (1 - z) * w + z * w * .8f, (1 - z) * h
					* .8f + z * h * .01f, w, (1 - z) * h * .9f + z * h * .01f);
			glaze.lineTo(w, h);
			glaze.closePath();
		}
		 ...
		 */
		protected void paintDropShadow(Graphics2D g, Component c,
				Map<String, Object> attributes) {
			float alpha = DecoratedPanelUI.Border.DROP_SHADOW_ALPHA.get(
					attributes, 0f);
			int layers = DecoratedPanelUI.Border.DROP_SHADOW_LAYER_COUNT.get(
					attributes, 0);
			...
			try {
				...
				for (float layer = 0; layer < layers; layer++) {
					attributes.put(Border.CURVE_LOWER_LEFT.toString(),
							Math.min(1, ll + .03f * layer));
					attributes.put(Border.CURVE_UPPER_LEFT.toString(),
							Math.min(1, ul + .03f * layer));
					attributes.put(Border.CURVE_LOWER_RIGHT.toString(),
							Math.min(1, lr + .03f * layer));
					attributes.put(Border.CURVE_UPPER_RIGHT.toString(),
							Math.min(1, ur + .03f * layer));
					attributes.put(Border.CORNER_SIZE_LOWER_LEFT.toString(),
							Math.min(maxSize, lls + layer));
					attributes.put(Border.CORNER_SIZE_LOWER_RIGHT.toString(),
							Math.min(maxSize, uls + layer));
					attributes.put(Border.CORNER_SIZE_UPPER_LEFT.toString(),
							Math.min(maxSize, lrs + layer));
					attributes.put(Border.CORNER_SIZE_UPPER_RIGHT.toString(),
							Math.min(maxSize, urs + layer));
					updateBodyShape(c, attributes, scratch, (int) (layer + .5));
					g2.translate(0, .5f);
					g2.setStroke(new BasicStroke(layer + .5f,
		...
		protected void paintBody(Graphics2D g, Component c,
			...
			if (c1 != null && c2 != null && !c1.equals(c2)) {
				Paint p = new GradientTexturePaint(new Color[] { c1, c2 },
						new float[] { 0, 1 }, new Point(0, 0), new Point(0,
								c.getHeight()), Cycle.TILE);
				/*
				 * Paint p = new GradientPaint( 0, 0, c1, 0, c.getHeight(), c2
				 * );
				 */
				g.setPaint(p);
				g.fill(body);
		...
		/**
		 * Paints everything in this renderer, after updating the body/glaze
		 * shapes. Specifically this invokes the paint methods in this order:
		 * <ol>
		 * <li>{@link #paintDropShadow(Graphics2D, Component, Map)}
		 ...
		 * <li>{@link #paintBorder(Graphics2D, Component, Map)}</li>
		 * </ol>
		 * 
		 * @param g
		 ...
		 * @param attributes
		 *            the available attributes, note some may be missing.
		 */
		...
		protected void paintBorder(Graphics2D g, Component c,
			...
			Paint borderPaint = Border.BORDER_PAINT
					.get(attributes, Color.black);

			if (borderWidth > 0) {
				g.setStroke(new BasicStroke(borderWidth, BasicStroke.CAP_BUTT,
		...
		protected void paintGlaze(Graphics2D g, Component c,
			...
			float glazeOpacity = DecoratedPanelUI.GLAZE_OPACITY.get(attributes,
					0f);
			...
			if (glazeOpacity > 0) {
				...
				try {
					...
					Paint glazeFill = new GradientPaint(0, (int) ((1 - z) * h
							* .7 + z * h * .3), new Color(0, 0, 0,
							(int) (glazeOpacity * 50)), 0, h, new Color(0, 0,
							0, 0));
					g2.clip(body);
		...
		protected void paintBevels(Graphics2D g, Component c,
			...
			int shadowStrokeCount = Border.BEVEL_SHADOW_LAYER_COUNT.get(
					attributes, 0);
			int highlightStrokeCount = Border.BEVEL_HIGHLIGHT_LAYER_COUNT.get(
					attributes, 0);
			float maxShadowBlend = Border.BEVEL_SHADOW_MAX_BLEND.get(
					attributes, 0f);
			float maxHighlightBlend = Border.BEVEL_HIGHLIGHT_MAX_BLEND.get(
					attributes, 0f);
			...
			try {
				...
				g2.translate(borderWidth * Math.cos(shadowTheta) / 2.0,
						borderWidth * Math.sin(shadowTheta) / 2.0);
				for (int a = 0; a < shadowStrokeCount; a++) {
					float f = ((float) (shadowStrokeCount - a))
							/ ((float) (shadowStrokeCount));
					g2.setColor(blendColors(new Color(0, 0, 0, 0), Color.black,
							f * maxShadowBlend));
			...
			try {
				...
				g2.translate(borderWidth * Math.cos(shadowTheta + Math.PI)
						/ 2.0, borderWidth * Math.sin(shadowTheta + Math.PI)
						/ 2.0);
				for (int a = 0; a < highlightStrokeCount; a++) {
					float f = ((float) (highlightStrokeCount - a))
							/ ((float) (highlightStrokeCount));
					g2.setColor(blendColors(new Color(255, 255, 255, 0),
							Color.white, f * maxHighlightBlend));
					g2.translate(shadowDX, shadowDY);
					g2.draw(body);
				}
			} finally {
	 ...
	 * <p>
	 * This border was originally designed to be used in combination with the
	 * <code>DecorationPanelUI</code>, but it can also be used independently as
	 * needed.
	 */
	public static class Border implements javax.swing.border.Border {
		...
		public static final Key<Float> STROKE_WIDTH = new Key<Float>(
				"stroke-width", Float.class, 0, 5, true, true);

		/**
		 * The attribute defining the upper-left corner's curvature, ranging
		 * from 0 (90 degrees) to 1 (circular curve).
		 */
		public static final Key<Float> CURVE_UPPER_LEFT = new Key<Float>(
				"curve-upper-left", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining the upper-right corner's curvature, ranging
		 * from 0 (90 degrees) to 1 (circular curve).
		 */
		public static final Key<Float> CURVE_UPPER_RIGHT = new Key<Float>(
				"curve-upper-right", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining the lower-left corner's curvature, ranging
		 * from 0 (90 degrees) to 1 (circular curve).
		 */
		public static final Key<Float> CURVE_LOWER_LEFT = new Key<Float>(
				"curver-lower-left", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining the lower-right corner's curvature, ranging
		 * from 0 (90 degrees) to 1 (circular curve).
		 */
		public static final Key<Float> CURVE_LOWER_RIGHT = new Key<Float>(
				"curve-lower-right", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining the upper-left corner's radius, ranging from 0
		 * to 100 pixels.
		 */
		public static final Key<Float> CORNER_SIZE_UPPER_LEFT = new Key<Float>(
				"corner-size-upper-left", Float.class, 0, 100, true, true);

		/**
		 * The attribute defining the upper-right corner's radius, ranging from
		 * 0 to 100 pixels.
		 */
		public static final Key<Float> CORNER_SIZE_UPPER_RIGHT = new Key<Float>(
				"corner-size-upper-right", Float.class, 0, 100, true, true);

		/**
		 * The attribute defining the lower-left corner's radius, ranging from 0
		 * to 100 pixels.
		 */
		public static final Key<Float> CORNER_SIZE_LOWER_LEFT = new Key<Float>(
				"corner-size-lower-left", Float.class, 0, 100, true, true);

		/**
		 * The attribute defining the lower-right corner's radius, ranging from
		 * 0 to 100 pixels.
		 */
		public static final Key<Float> CORNER_SIZE_LOWER_RIGHT = new Key<Float>(
				"corner-size-lower-right", Float.class, 0, 100, true, true);

		/**
		 * The attribute defining the border paint, which is only made visible
		 * if {@link #STROKE_WIDTH} is nonzero.
		 */
		public static final Key<Paint> BORDER_PAINT = new Key<Paint>(
				"border-paint", Paint.class);

		/**
		 * The attribute defining the angle the bevel shadows and highlights are
		 * oriented, ranging from 0 to 2*pi. A value of zero positions the
		 * shadow on the left side, and it rotates clockwise from there: so a
		 * value of pi/2 puts the shadow on the top and a value of pi puts the
		 * shadow on the right. The highlight is always opposite the shadow.
		 */
		public static final Key<Float> BEVEL_SHADOW_THETA = new Key<Float>(
				"bevel-shadow-theta", Float.class);

		/**
		 * The attribute defining the number of paint operations used to layer
		 * the bevel shadow, ranging from 0 to 20.
		 */
		public static final Key<Integer> BEVEL_SHADOW_LAYER_COUNT = new Key<Integer>(
				"bevel-shadow-layer-count", Integer.class, 0, 20, true, true);

		/**
		 * The attribute defining the number of paint operations used to layer
		 * the bevel highlight, ranging from 0 to 20.
		 */
		public static final Key<Integer> BEVEL_HIGHLIGHT_LAYER_COUNT = new Key<Integer>(
				"bevel-highlight-layer-count", Integer.class, 0, 20, true, true);

		/**
		 * The attribute defining how dark to make the shadow bevel, ranging
		 * from 0 to 1.
		 */
		public static final Key<Float> BEVEL_SHADOW_MAX_BLEND = new Key<Float>(
				"bevel-shadow-max-blend", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining how light to make the highlight bevel, ranging
		 * from 0 to 1.
		 */
		public static final Key<Float> BEVEL_HIGHLIGHT_MAX_BLEND = new Key<Float>(
				"bevel-highlight-max-blend", Float.class, 0, 1, true, true);

		/**
		 * The attribute defining the number of paint operations used to layer
		 * the drop shadow, ranging from 0 to 20.
		 */
		public static final Key<Integer> DROP_SHADOW_LAYER_COUNT = new Key<Integer>(
				"drop-shadow-layer-count", Integer.class, 0, 20, true, true);

		/**
		 * The attribute defining how dark to make each drop shadow layer,
		 * ranging from 0 to .05.
		 */
		public static final Key<Float> DROP_SHADOW_ALPHA = new Key<Float>(
				"drop-shadow-alpha", Float.class, 0, .05f, true, true);

		/**
		 * The attribute defining how many pixels to deviate from the ideal body
		 * outline when applying a scribble effect, ranging from 0 to 5 pixels.
		 */
		public static final Key<Float> SCRIBBLE_SIZE = new Key<Float>(
				"scribble-size", Float.class, 0, 5, true, true);
		...
		public <T> T getProperty(Key<T> key) {
			if (!isSupported(key))
				throw new IllegalArgumentException(
						"The Border class does not supported \"" + key + "\"");
		 ...
		 * Paint this border.
		 * <p>
		 * This asks a {@link Renderer} to paint everything, but first uses a
		 * clipping to punch a whole in the middle of the Graphics2D. (So only
		 * the border will show.)
		 * <p>
		 * If the component has applied a <code>DecoratedPanelUI</code>: then
		 * attributes from that panel are also retrieved so the
		...
		public Insets getBorderInsets(Component c) {
			...
			right = (int) (w
					+ Math.max(getUpperRightCornerSize(),
							getLowerRightCornerSize()) + .5f);
			bottom = (int) (w
					+ Math.max(getLowerLeftCornerSize(),
							getLowerRightCornerSize()) + .5f);
			top = (int) (w
					+ Math.max(getUpperRightCornerSize(),
							getUpperLeftCornerSize()) + .5f);
			left = (int) (w
					+ Math.max(getLowerLeftCornerSize(),
							getUpperLeftCornerSize()) + .5f);
			Insets curvatureInsets = new Insets(top, left, bottom, right);

			float bevel = Math.max(getBevelShadowLayerCount(),
					getBevelHighlightLayerCount());
			int k = (int) (w + bevel + 2 * getScribbleSize() + .5)
					+ getDropShadowLayerCount();
	...
	public static final Key<Float> GLAZE_LEVEL = new Key<Float>("glaze-level",
			Float.class, 0, 1, true, true);

	/** The attribute defining how opaque make the glaze, ranging from 0 to 1. */
	public static final Key<Float> GLAZE_OPACITY = new Key<Float>(
			"glaze-opacity", Float.class, 0, 1, true, true);

	/**
	 * The attribute defining the top color of the vertical gradient. If this is
	 * null, then only the COLOR_BOTTOM is used. If that is also null, then the
	 * panel background is used. (And if that is null or
	 * <code>panel.isOpaque()</code> is false, then the body is left empty.
	 */
	public static final Key<Color> COLOR_TOP = new Key<Color>("color-top",
			Color.class);

	/**
	 * The attribute defining the bottom color of the vertical gradient. If this
	 * is null, then only the COLOR_TOP is used. If that is also null, then the
	 * panel background is used. (And if that is null or
	 * <code>panel.isOpaque()</code> is false, then the body is left empty.
	 */
	public static final Key<Color> COLOR_BOTTOM = new Key<Color>(
			"color-bottom", Color.class);
	...
	/**
	 * Create a new <code>DecoratedPanelUI</code> with a default
	 * {@link Renderer}.
	 */
	...
	public <T> T getProperty(Key<T> key) {
		if (!isSupported(key))
			throw new IllegalArgumentException(
					"The DecoratedPanelUI class does not supported \"" + key
							+ "\"");
		return properties.get(key);
	}
	 ...
	 * installed when <code>panel.setUI(this)</code> is called. (Or more
	 * specifically: this is automatically installed when
	 * <code>this.installUI(panel)</code> is called.)
	 */
	 ...
	 * This paints the UI.
	 * <p>
	 * This method relies on the {@link Renderer} class to paint everything, but
	 * first it clips the Graphics2D to hide the border (so the border will
	 * render separately).
	 */
	@Override
	public void paint(Graphics g, JComponent c) {
		...
		try {
			if (c.getBorder() == border) {
				...
				g2.clipRect(i.left, i.top, c.getWidth() - i.left - i.right,
	...
	protected Map<String, Object> getAllProperties() {
		Map<String, Object> r = properties.getMap(false, false,
				ObservableProperties.DEFAULT);
		r.putAll(border.properties.getMap(false, false,
				ObservableProperties.DEFAULT));
		return r;
	}
	...
	/**
	 * Set the top and bottom colors to the same value, making this a uniform
	 * color.
	 * 
	 * @see #COLOR_TOP
	 * @see #COLOR_BOTTOM
	 ...
	 *            the second color
	 * @param p
	 *            the progress (as a decimal between zero and one) to use. When
	 *            p is zero, the color returned is "a". When p is one, the color
	 *            returned is "b". When p is .5, the color is 50% "a" and 50%
	 *            "b", etc.
	...
	public static Color blendColors(Color a, Color b, float p) {
		if (p < 0 || p > 1)
			throw new IllegalArgumentException("p (" + p
					+ ") must be between [0,1]");
		return new Color((int) (a.getRed() * (1 - p) + b.getRed() * p),
