...
public class AquaAngleSliderUI extends AngleSliderUI {
	...
	protected void calculateGeometry(JSlider slider) {
		...
		slider.putClientProperty(MY_CACHED_FILLS, null); // force these to
															// recalculate
	}

	/**
	 * The dial is simply a series of translucent linear gradients places on top
	 * of each other. This method calculates those gradients.
	 */
	private Paint[] getFills(JSlider slider) {
		...
		if (dark) {
			p[0] = new GradientPaint(0, y + 1f / 5f * h, blue1, 0, y + h - h
					/ 4, blue2);
		} else {
			p[0] = new GradientPaint(0, y + 1f / 5f * h, gray1, 0, y + h - h
					/ 4, gray2);
		}
		...
		p[1] = new GradientPaint(x, y + h / 2, shadow1, x + w / 4, y + h / 2,
				shadow3);
		p[2] = new GradientPaint(x + w, y + h / 2, shadow1, x + w - w / 4, y
				+ h / 2, shadow3);

		p[3] = new GradientPaint(0, y + 1f / 10f * h, highlight1, 0, y + 1f
				/ 7f * h, highlight2);

		p[4] = new GradientPaint(x, y + h / 2 - h / 5, shadow2, x + w / 4, y
				+ h / 2, shadow3);
		p[5] = new GradientPaint(x + w, y + h / 2 - h / 5, shadow2, x + w - w
				/ 4, y + h / 2, shadow3);
	...
	public void paint(Graphics g0, JComponent c) {
		...
		float r = (float) Math.min(centerX - data.dial.getX(), centerY
				- data.dial.getY()) - 4;
