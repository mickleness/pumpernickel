...
public class DebugPanel extends JPanel {
	...
	protected void paintChildren(Graphics g0) {
		...
		if (c != null && c != this) {
			...
			g.setStroke(new BasicStroke(1, BasicStroke.CAP_BUTT,
					BasicStroke.JOIN_MITER, 10, new float[] { 4, 4 }, 4));
			for (int i = 0; i < getComponentCount(); i++) {
				Component c2 = getComponent(i);
				if (c2 != null && c2 != c) {
					boolean intercepts = false;
					for (int j = 0; j < baselines.length; j++) {
						if (baselines[j] >= c2.getY()
								&& baselines[j] <= c2.getY() + c2.getHeight()) {
							intercepts = true;
						}
					}
					if (intercepts) {
						...
						for (int j = 0; j < baselines2.length; j++) {
							...
							if (contains(baselines, baselines2[j]) == false) {
								g.drawLine(0, baselines2[j], getWidth(),
										baselines2[j]);
							}
						}
					}
			...
			if (i != null) {
				...
				g.fillRect(c.getX(), c.getY() + c.getHeight() - i.bottom,
						c.getWidth(), i.bottom);
				g.fillRect(c.getX(), c.getY() + i.top, i.left, c.getHeight()
						- i.top - i.bottom);
				g.fillRect(c.getX() + c.getWidth() - i.right, c.getY() + i.top,
						i.right, c.getHeight() - i.top - i.bottom);
			}
		}
	}
