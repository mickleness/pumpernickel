...
@Blurb(title = "Performance: Studying Math", releaseDate = "May 2009", summary = "The <code><a href=\"http://java.sun.com/javase/6/docs/api/java/lang/Math.html\">java.lang.Math</a></code> "
		...
		+ "<p>But if you're dealing with graphics: you can probably fudge on some accuracy in exchange for speed.  What methods can be improved?", article = "http://javagraphics.blogspot.com/2009/05/math-studying-performance.html")
public abstract class MathG {
	 ...
	 * 
	 * @param d
	 *            the value to calculate the ceil of.
	 ...
	 * @param d
	 *            the value to calculate the ceil of.
	 * @return the closest integer that is greater than the argument as a
	 *         double.
	 */
	...
	private static Function sinFunction01 = PolynomialFunction.createFit(
			...
			new double[] { Math.sin(0), Math.sin(Math.PI / 2) }, new double[] {
					Math.cos(0), Math.cos(Math.PI / 2) });

	private static Function sinFunction00004 = PolynomialFunction.createFit(
			new double[] { 0, Math.PI / 4, Math.PI / 2 },
			new double[] { Math.sin(0), Math.sin(Math.PI / 4),
					Math.sin(Math.PI / 2) },
			new double[] { Math.cos(0), Math.cos(Math.PI / 4),
					Math.cos(Math.PI / 2) });
	...
	static {
		...
		p.setFunction(
				p.getFunctionCount() - 1,
				PiecewiseFunction.create(acos, acosD,
						1.0 - 1.0 / (p.getFunctionCount()), 1, 64));
		acosFunction = p;
	}

	/**
	 * Returns an approximate value of the sin(v) that should be within
	 * plus-or-minus .0108 of the value returned by Math.sin().
	 * <P>
	 * If the argument is greater in magnitude than 1e10, then this delegates to
	 * Math.sin().
	 * 
	 * @param v
	...
	public static final double sin01(double v) {
		/**
		 * This is exactly the same code as sin00004, except it uses the smaller
		 * polynomial. I avoided refactoring the code to use a common method in
		 * the theory that avoiding adding a method to the stack trace may shave
		 * off just a tiny bit of performance. Normally this would be excessive,
		 * but my goal is optimum performance in really tight loops: every line
		 * counts!
		 */
		...
		if (v > 1.0E10) {
			if (printedOverflowError == false) {
				...
				System.err
						.println("Warning: MathG is not designed to estimate the sine of values of 1.0e10.  Math.sin() will be used, which may result in slower performance.");
			}
	...
	/**
	 * Controls whether an error message has been printed to System.err. yet
	 * this session regarding calling calculateSin() on values that are too
	 * large.
	 */
	private static boolean printedOverflowError = false;

	/**
	 * Returns an approximate value of the cos(v) that should be within
	 * plus-or-minus .0108 of the value returned by Math.cos().
	 * <P>
	 * If the argument is greater in magnitude than 1e10, then this delegates to
	 * Math.cos().
	 * 
	...
	/**
	 * Returns an approximate value of the sin(v) that should be within
	 * plus-or-minus .00004 of the value returned by Math.sin().
	 * <P>
	 * If the argument is greater in magnitude than 1e10, then this delegates to
	 * Math.sin().
	 * 
	...
	/**
	 * Returns an approximate value of the acos(v) that should be within
	 * plus-or-minus .00004 of the value returned by Math.acos().
	 * 
	 * @param v
	...
	/**
	 * Returns an approximate value of the cos(v) that should be within
	 * plus-or-minus .00004 of the value returned by Math.cos().
	 * <P>
	 * If the argument is greater in magnitude than 1e10, then this delegates to
	 * Math.cos().
	 * 
