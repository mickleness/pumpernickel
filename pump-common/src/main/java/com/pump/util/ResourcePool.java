...
/**
 * This object manages a pool of resources (arrays and images) for frequent
 * repetitive reuse.
 * <p>
 * Sometimes constantly reallocating resources in tight loops can be an
 * expensive drain, so this should help relieve that performance expense.
 * <p>
 * This pool also has limitations, though, so it does not introduce a memory
 * leak. Strong references to resources are purged after a fixed time interval
 * (usually 5 seconds). Also there are limits to the number of resources of any
 * given type that will be stored. There are two such limits:
 * <ul>
 * <li>A limit of the exact resource type. For example: by default there will
 * only be 5 images of the same width, height and type.</li>
 * <li>A limit of all resources of a given type. For example: by default there
 * will only be 20 images total.</li>
 * </ul>
 * <p>
 * (Arrays have a higher tolerance.)
 * <p>
 * It is still possible to abuse this model and introduce memory problems, in
 * the same way that is possible to run out of memory just by constructing a few
 * 10,000x10,000 images. But when used responsibly, this should be a valuable
 * tool to easy the cost of constructing thousands of similar objects.
 * <p>
 * Every time you retrieve an object from this pool, you should wrap the
 * following code in a try/finally block and return the object back to this pool
 * when finished.
 * 
 * @see com.pump.showcase.ResourcePoolDemo
 */
@Blurb(title = "Performance: Constructing Arrays and Images", releaseDate = "April 2014", summary = "This examines the cost of constructing arrays and BufferedImages, vs caching and reusing existing arrays.\n"
		+ "<p>This is intended to be used for highly repetitive tasks (such as batch image processing or rendering work).", article = "http://javagraphics.blogspot.com/2014/04/performance-constructing-arrays-and.html")
public class ResourcePool {

	private enum Type {
		IMAGE, FLOAT, INT, DOUBLE, SHORT, LONG, BYTE
	};
	...
	public static class Limit {
		 ...
		 * 
		 * @param groupLimit
		 *            the number of identical instances this pool stores.
		 * @param totalLimit
		 *            the total number of objects of a given type this stores.
		 * @param timeLimit
		 *            the number of seconds this stores a resource.
		 */
			...
			public void run() {
				while (true) {
					synchronized (allPools) {
						Iterator<WeakReference<ResourcePool>> iter = allPools
								.iterator();
						while (iter.hasNext()) {
	...
	/**
	 * Iterate over all resources and make sure nothing has expired based on the
	 * current time limits and time stamps.
	 * 
	 */
	...
	/**
	 * Return a BufferedImage from this pool. If no cached images exists
	 * matching these parameters: then a new image is created.
	 * 
	 * @param width
	 ...
	 * @param clear
	 *            if true and a cached image is identified: then that image is
	 *            first cleared (using an AlphaComposite.Clear).
	 * @return an image matching the arguments provided. If possible this will
	 *         recycle a previously cached image.
	 */
	public synchronized BufferedImage getImage(int width, int height, int type,
			boolean clear) {
	...
	/**
	 * Return an int array from this pool. If no cached int array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	 */
	...
	/**
	 * Return a float array from this pool. If no cached float array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	...
	/**
	 * Return a double array from this pool. If no cached double array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	...
	/**
	 * Return a short array from this pool. If no cached short array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	...
	/**
	 * Return a long array from this pool. If no cached long array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	...
	/**
	 * Return a byte array from this pool. If no cached byte array exists
	 * matching the requested length: then a new array is created.
	 * 
	 * @param length
	 *            the array length
	 ...
	 * 
	 * @return true if the array was stored, false if this pool is not currently
	 *         accepting any more arrays of this type/size.
	 */
	 ...
	 * 
	 * @return true if the image was stored, false if this pool is not currently
	 *         accepting any more images of this type/size.
	 */
	 ...
	 * 
	 * @return true if the resource was stored, false if the current Limits do
	 *         not allow storing more objects of this type/id
	 */
