 ...
 * <p>
 * Many browsers consistently render popups in the wrong location: they take the
 * (x, y) of the applet but display the popup menu relative to the browser
 * window instead of the applet -- so it is displaced up and to the left.
 * <p>
...
public class AppletPopupFactory extends PopupFactory {
	...
	static class AppletPopup extends Popup {
		...
		class MyBlockingPane extends BlockingPane {
			...
			protected void paintComponent(Graphics g0) {
				...
				if (backgroundColor != null) {
					...
					for (int a = 0; a < 10; a++) {
						g2.setStroke(new BasicStroke(a + 1,
								BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
						g2.drawRect(rect.x, rect.y + a / 2, rect.width,
								rect.height);
					}
					g2.dispose();
					...
					g.fillRect(rect.x - 1, rect.y - 1, rect.width + 1,
							rect.height + 1);
				}
				if (borderColor != null) {
					g.setColor(borderColor);
					g.drawRect(rect.x - 1, rect.y - 1, rect.width + 1,
							rect.height + 1);
				}
			}
		};
		...
		public AppletPopup(JApplet applet, Component owner,
			...
			/*
			 * For some bizarre reason: the LineBorder doesn't show up like it
			 * should. So we extract it and let the blocking pane paint it
			 * instead.
			 */
			if (component.getBorder() instanceof LineBorder) {
				borderColor = ((LineBorder) component.getBorder())
						.getLineColor();
				component.setBorder(null);
			}

			/**
			 * Extract the background color to paint under the popup. This is
			 * custom-designed to work with Mac's and the AquaComboBoxUI.
			 */
		...
		public void show() {
			...
			if (component.getParent() != applet.getLayeredPane()) {
				applet.getLayeredPane()
						.add(component, JLayeredPane.POPUP_LAYER);
			}
	 ...
	 * 
	 * @param oldFactory
	 *            we defer to this factory when we don't have to use the fake
	 *            popup model.
	 */
	...
	public Popup getPopup(Component owner, Component contents, int x, int y)
		...
		for (int a = hierarchy.length - 1; a >= 0; a--) {
			if (hierarchy[a] instanceof JApplet
				...
				return new AppletPopup((JApplet) hierarchy[a], owner,
						(JComponent) contents, x, y);
