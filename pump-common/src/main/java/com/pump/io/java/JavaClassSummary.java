 ...
 *
 * $Date: 2016-06-20 20:01:41 +0500 (Mon, 20 Jun 2016) $
 *
...
import com.pump.io.java.JavaParser.BracketCharToken;
import com.pump.io.java.JavaParser.BracketType;
import com.pump.io.java.JavaParser.DeclarationType;
import com.pump.io.java.JavaParser.JavaModifier;
...
public class JavaClassSummary
{
	...
	class MyReceiver implements Receiver<Token> {
		...
		public void add(Token... tokens) {
			for(Token token : tokens) {
				if(token.getText().equals(";")) {
				...
				} else if(brackets.size()==0 && token instanceof WordToken && ((WordToken)token).isDeclarationType) {
					declarationType = DeclarationType.valueOf(token.getText().toUpperCase());
				} else if(brackets.size()==0 && declarationType!=null && simpleName==null && token instanceof WordToken) {
					...
					if(bct.isOpen()) {
						...
						if(brackets.size()>0 && brackets.peek()==bct.getBracketType()) {
							brackets.pop();
						} else {
	...
	public JavaClassSummary(File javaFile) throws MalformedURLException, IOException {
	...
	public static String getClassName(File sourceCodeFile) throws IOException {
		...
		if(!filename.endsWith(".java")) {
			throw new IllegalArgumentException("this file is not a .java file: "+sourceCodeFile.getAbsolutePath());
		}
