...
public class ClassWriterTest extends TestCase {
	...
	/**
	 * This is modeled after a compiler issue observed in the ObservableSet when
	 * x-ray first tried to autogenerate those signatures.
	 * <p>
	 * Apparently have a static parameterized class and a nested non-static
	 * parameterized class <em>requires</em> we use the simple class name (in
	 * this case: "Operation") instead of the fully qualified class name
	...
	public void assertEquals(Class type, String resourceName) throws Exception {
		...
		try {
			...
			if (type.getDeclaringClass() == null) {
				...
				Class z = type.getDeclaringClass() == null ? type : type
						.getDeclaringClass();
			...
			System.err.println("Failure for \"" + resourceName + "\" ("
					+ type.getName() + ")");
