...
public class ListEncoder<S> extends ValueEncoder<List<S>> {
	...
	public ListEncoder(Class<S> elementType) {
		...
		if (elementEncoder == null)
			throw new NullPointerException("There is no default encoder for "
					+ elementType.getName()
					+ ". Please provide an explicit encoder.");
	}
	...
	public List<S> parse(String str) {
		...
		if (elementEncoder.getType().equals(String.class)) {
			...
			// we can searh opened brackets/parentheses, then quotes, then find
			// their closing pairs, etc.
		...
		while (i < str.length()) {
			...
			if (ch != '"')
				throw new IllegalArgumentException(
						"Unexpected character at start of phrase: " + ch);
			i++;
			...
			readValue: while (true) {
				...
				if (str.substring(i, j).equals("\"")) {
					returnValue
							.add(sb.substring(0, sb.length() - 1).toString());
					sb = new StringBuffer();
					i = j;
					break readValue;
				}
			...
			if (ch != ',')
				throw new IllegalArgumentException(
						"Unexpected character after phrase: " + ch);
			i++;
		}
