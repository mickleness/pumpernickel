...
/**
 * This paints a {@link FillInstruction} and then a {@link DrawInstruction} on
 * top of it. This requires the shape, clipping, transform, and opacities of the
 * fill and the stroke be the same.
 *
...
public class BasicShapeInstruction extends BasicFillInstruction implements
	 ...
	 * @param clipping
	 *            the optional clipping (may be null).
	 * @param fillPaint
	 *            the fill paint.
	 * @param strokePaint
	 *            the stroke paint.
	 ...
	 *            the opacity [0,1].
	 */
	public BasicShapeInstruction(Shape shape, AffineTransform transform,
			Shape clipping, Paint fillPaint, Paint strokePaint, Stroke stroke,
			float opacity) {
	...
	/**
	 * Returns a ShapeInstruction if possible that embodies both argument
	 * instructions. If not possible (because the fill, transform, or clipping
	 * is a problem), then this returns null.
	 * 
	 * @param fillInstr
	 * @param drawInstr
	 * @return a new instruction representing both argument instructions, or
	 *         null if it isn't possible to combine these instructions.
	 */
	public static BasicShapeInstruction merge(FillInstruction fillInstr,
		...
		return new BasicShapeInstruction(shape1, fillInstr.getTransform(),
				fillInstr.getClipping(), fillInstr.getFillPaint(),
				drawInstr.getStrokePaint(), drawInstr.getStroke(),
				fillInstr.getOpacity());
	}

	/** Extracts an instruction that only fills this shape. */
	public FillInstruction getFillInstruction() {
		return new BasicFillInstruction(getShape(), getTransform(),
				getClipping(), getFillPaint(), getOpacity());
	}

	/** Extracts an instruction that only draws this shape. */
	public DrawInstruction getDrawInstruction() {
		return new BasicDrawInstruction(getShape(), getTransform(),
				getClipping(), getStrokePaint(), getStroke(), getOpacity());
	}
	...
	private void readObject(java.io.ObjectInputStream in) throws IOException,
		...
		if (strokeObject instanceof String) {
			stroke = BasicDrawInstruction
					.createBasicStroke((String) strokeObject);
		} else {
