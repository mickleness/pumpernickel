...
class CompositeShadowRendererGenerator implements Callable<CompositeShadowRendererGenerator.Results> {
	...
	public static void main(String[] args) throws Exception {
		...
		for (BigDecimal gaussianRadius = gaussianIncr; gaussianRadius
				.compareTo(gaussianMax) <= 0; gaussianRadius = gaussianRadius.add(gaussianIncr)) {
	...
	static class Results implements Comparable<Results> {
		...
		public Results(long error, Combo combo, ARGBPixels pixels, BigDecimal value, BigDecimal incr,
				BigDecimal gaussianRadius, BufferedImage gaussianShadowImage) {
			this.error = error;
			this.combo = combo;
			this.pixels = pixels;
			this.value = value;
		...
		String output() throws Exception {
			...
			ImageIO.write(pixels.createBufferedImage(true), "png", new File(name + "-" + combo.sortedRadii + ".png"));
	...
	public Results call() {
		try {
			...
			while (results.incr.compareTo(fastIncr) >= 0) {
				...
				results = getBestFit(results.value.subtract(results.incr.multiply(BigDecimal.valueOf(2))),
						results.value.add(results.incr.multiply(BigDecimal.valueOf(2))), newIncr);
			}
	...
	private int[] scratchPaper = null;
	private Results getBestFit(BigDecimal min, BigDecimal max, BigDecimal incr) throws Exception {
		...
		for (BigDecimal v1 = min; v1.compareTo(max) <= 0; v1 = v1.add(incr)) {
			for (BigDecimal v2 = v1; v2.compareTo(max) <= 0; v2 = v2.add(incr)) {
				//for (BigDecimal v3 = v2; v3.compareTo(max) <= 0; v3 = v3.add(incr)) {
					Combo combo = new Combo(v1.floatValue(), v2.floatValue()); //), v3.floatValue());
					int width = 2*combo.radiiSum + starPixels.getWidth();
					int height = 2*combo.radiiSum + starPixels.getHeight();
					int requiredArraySize = width * height;
					if(scratchPaper==null || scratchPaper.length<requiredArraySize) {
						scratchPaper = new int[requiredArraySize];
					} else {
						Arrays.fill(scratchPaper, 0);
					}
					ARGBPixels shadowPixels = new ARGBPixels(width, height, scratchPaper);
					combo.createShadow(starPixels, shadowPixels, SHADOW_OPACITY);
					long error = getError(gaussianShadowImage, shadowPixels.createBufferedImage(true));
					Results results = new Results(error, combo, shadowPixels, v1, incr, gaussianRadius,
							gaussianShadowImage);
	...
	private static BufferedImage createStar() {
		BufferedImage starImage = new BufferedImage(400, 400, BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = starImage.createGraphics();
		StarPolygon p = new StarPolygon(1);
		g.setColor(Color.black);
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		g.setTransform(TransformUtils.createAffineTransform(p.getBounds2D(), new Rectangle(10, 10, 380, 380)));
		g.fill(p);

		// punch a whole in the center:
		g.setComposite(AlphaComposite.Clear);
		AffineTransform tx = AffineTransform.getRotateInstance(.4, 200, 200);
		tx.concatenate(TransformUtils.createAffineTransform(p.getBounds2D(), new Rectangle(100, 100, 200, 200)));
		g.setTransform(tx);
		g.fill(p);
