...
public class DesktopApplication {
	 ...
	 * @param supportEmail
	 *            if non-null then error dialogs will include a "Report..."
	 *            button to help users email tech support about exceptions that
	 *            occur during a session.
	 * @param frameClass
	 *            if non-null then we'll add a listener that guarantees to exit
	 *            the app after all Frames of the type frameClass are closed.
	 *            (This does not in any way help manage saving dirty windows,
	 *            this just makes sure the process doesn't silently linger after
	 *            the user has closed all UI components.) As a default you can
	 *            just use Frame.class here, but if you are focused on a very
	 *            specific extension of JFrame you can use that subclass.
	 * @throws IOException
	 */
	public static void initialize(final String qualifiedAppName,
		...
		System.out.println("DesktopApplication.initialize( " + qualifiedAppName
				+ ", " + simpleAppName + ", " + version + ", " + supportEmail
