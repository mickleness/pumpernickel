...
class InspectorLayoutPlacement {
	...
	/**
	 * The minimum column width for all leading components (the left-most column)
	 */
	...
	public InspectorLayoutPlacement(Inspector inspector,JPanel parent) {
		...
		borderInsets = border==null ? new Insets(0,0,0,0) : border.getBorderInsets(parent);
		...
		for(int a = 0; a<rows.length; a++) {
			...
			rowInfos[a].borderInsets = rowBorder==null ? new Insets(0,0,0,0) : rowBorder.getBorderInsets(row);
			...
			Insets leadInsets = lead==null ? new Insets(0,0,0,0) : inspector.getInsets(lead, Position.LEAD);
			Insets mainInsets = main==null ? new Insets(0,0,0,0) : inspector.getInsets(main, Position.MAIN_ONLY);
			...
			if(leadBaseline>0 && mainBaseline>0) {
				rowInfos[a].sharedBaseline = Math.max(leadBaseline + leadInsets.top, mainBaseline + mainInsets.top) + rowInfos[a].borderInsets.top;
				rowInfos[a].leadVerticalPadding = rowInfos[a].sharedBaseline - leadBaseline;
				rowInfos[a].mainVerticalPadding = rowInfos[a].sharedBaseline - mainBaseline;
			}
			...
			if(leadSize!=null && mainSize!=null) {
				rowInfos[a].minimumRowHeight = Math.max(leadSize.height + rowInfos[a].leadVerticalPadding + leadInsets.top + leadInsets.bottom, 
						mainSize.height + rowInfos[a].mainVerticalPadding + mainInsets.top + mainInsets.bottom) +
						rowInfos[a].borderInsets.top + rowInfos[a].borderInsets.bottom;
				leadWidth = Math.max(leadWidth, leadSize.width + leadInsets.left + leadInsets.right + rowInfos[a].borderInsets.left);
				mainWidth = Math.max(mainWidth, mainSize.width + mainInsets.left + mainInsets.right + rowInfos[a].borderInsets.right);
			} else if(leadSize!=null) {
				rowInfos[a].minimumRowHeight = leadSize.height + rowInfos[a].leadVerticalPadding + leadInsets.top + leadInsets.bottom + rowInfos[a].borderInsets.top + rowInfos[a].borderInsets.bottom;
				leadWidth = Math.max(leadWidth, leadSize.width + leadInsets.left + leadInsets.right + rowInfos[a].borderInsets.left);
			} else if(mainSize!=null) {
				rowInfos[a].minimumRowHeight = mainSize.height + rowInfos[a].mainVerticalPadding + mainInsets.top + mainInsets.bottom + rowInfos[a].borderInsets.top + rowInfos[a].borderInsets.bottom;
				//if the lead is null the main component gets the full width:
				totalWidth = Math.max(totalWidth, mainSize.width + mainInsets.left + mainInsets.right + borderInsets.left + borderInsets.right + rowInfos[a].borderInsets.left + rowInfos[a].borderInsets.right);
			} else {
			...
			totalWidth = Math.max(totalWidth, leadWidth + mainWidth + borderInsets.left + borderInsets.right);
	...
	private InspectorRowPanel[] getRows(JPanel parent) {
		List<InspectorRowPanel> rows = new ArrayList<>(parent.getComponentCount());
	...
	public void install() {
		...
		int rowWidth = parentSize.width - borderInsets.left - borderInsets.right;
	...
	private void installRow(int rowIndex) {
		...
		if(lead!=null) {
			...
			if(i.sharedBaseline > 0) {
				...
				y = (row.getHeight() - i.borderInsets.top - i.borderInsets.bottom)/2 - leadSize.height/2 + i.borderInsets.top;
			}
			...
			if(main!=null) {
				...
				int width;
				int extraWidth = mainWidth - d.width - mainInsets.left - mainInsets.right - i.borderInsets.right;
				float horizWeight = row.getInspectorRow().getMainComponentHorizontalWeight();
				...
				if(i.sharedBaseline > 0) {
					...
					y = (row.getHeight() - i.borderInsets.top - i.borderInsets.bottom)/2 - d.height/2 + i.borderInsets.top;
				}
				main.setBounds(leadWidth - 1 + mainInsets.left, y, width, d.height);
			...
			int width;
			float horizWeight = row.getInspectorRow().getMainComponentHorizontalWeight();
			int extraWidth = row.getWidth() - i.borderInsets.left - i.borderInsets.right - d.width - mainInsets.left - mainInsets.right;
			if(extraWidth < 0 || horizWeight==0) {
				width = d.width;
			} else {
				width = d.width + (int)(horizWeight * extraWidth);
			...
			main.setBounds(mainInsets.left + i.borderInsets.left, mainInsets.top + i.borderInsets.top, width, d.height);
		}
	}
}
