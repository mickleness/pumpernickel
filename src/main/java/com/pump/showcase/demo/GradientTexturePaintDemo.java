...
public class GradientTexturePaintDemo extends ShowcaseExampleDemo {
		...
		protected void paintComponent(Graphics g0) {
					...
					BufferedImage.TYPE_INT_ARGB);
			{
				...
				CycleMethod cycle = (CycleMethod) cycleComboBox
						.getSelectedItem();
				if (typeComboBox
						.getSelectedItem() == GradientType.GRADIENT_TEXTURE_PAINT) {
					p = new GradientTexturePaint(new Color[] { c1, c2 },
							new float[] { 0, 1 }, p1, p2, cycle);
				} else if (typeComboBox
						.getSelectedItem() == GradientType.GRADIENT_PAINT) {
			...
			for (int y = 0; y < getHeight(); y++) {
				...
				for (int x = 0; x < d.length; x++) {
					...
					Color c = AnimationManager.tween(
							well1.getColorSelectionModel().getSelectedColor(),
							well2.getColorSelectionModel().getSelectedColor(),
							f);
					d[x] = c.getRGB();
				}
	...
	public GradientTexturePaintDemo() {
		...
		ActionListener actionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				unsupportedLabel.setVisible(typeComboBox
						.getSelectedItem() == GradientType.GRADIENT_PAINT);
	...
	public Class<?>[] getClasses() {
		return new Class[] { GradientTexturePaintDemo.class, TexturePaint.class,
				GradientPaint.class };
	}
}