 ...
 */
public class WildcardPatternHighlighterDemo implements ShowcaseDemo {

	static class WildcardPatternHighlighterDemoPanel extends JPanel {
		...
		JSpinner thicknessSpinner = new JSpinner(new SpinnerNumberModel(1, 1,
				3, 1));
		...
		JSpinner hueSpinner = new JSpinner(new SpinnerNumberModel(180, 1, 360,
				5));
		...
		JSpinner alphaSpinner = new JSpinner(new SpinnerNumberModel(100, 1,
				100, 1));
		...
		public WildcardPatternHighlighterDemoPanel() {
			...
			underlinePatternField.getDocument()
			...
			for (Word word : words) {
				if (pattern.matches(word)) {
					textPane.getHighlighter().addHighlight(
							word.getStartIndex(), word.getEndIndex(), painter);
				}
			}
		}
	...
	public JPanel createPanel(PumpernickelShowcaseApp psa) {
		return new WildcardPatternHighlighterDemoPanel();
	}
