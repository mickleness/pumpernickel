 ...
 */
public abstract class TransitionDemo implements ShowcaseDemo {
	static class TransitionDemoPanel extends JPanel {
		...
		public TransitionDemoPanel(Transition[][] transitions) {
			this(AbstractTransition.createImage("A", true), AbstractTransition
					.createImage("B", false), transitions);
		}

		public TransitionDemoPanel(BufferedImage bi1, BufferedImage bi2,
			...
			transitionFamilyComboBox.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					String familyName = (String) transitionFamilyComboBox
							.getSelectedItem();
					if (familyName != null) {
						List<Transition> t = transitionsByFamily
								.get(familyName);
			...
			layout.addRow(new JLabel("Transition Type:"),
					transitionFamilyComboBox, false);
			layout.addRow(new JLabel("Transition:"), transitionComboBox, false);
			layout.addRow(new JLabel("Duration (s):"), duration, false);
			layout.addRow(new JLabel("Rendering Hints"),
					renderingHintsComboBox, false);
		...
		/**
		 * Given several Transitions, this identifies the run of words they have
		 * in common. Sometimes there will only be one word (like "Bars"), but
		...
		class TransitionPanel extends BufferedAnimationPanel {
			...
			public TransitionPanel(Transition transition) {
				...
				setPreferredSize(new Dimension(img1.getWidth(),
						img1.getHeight()));
			}
	...
	public JPanel createPanel(PumpernickelShowcaseApp psa) {
		return new TransitionDemoPanel(getTransitions());
	}
