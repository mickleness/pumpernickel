...
public abstract class ShowcaseChartDemo implements ShowcaseDemo {

	class ShowcaseChartDemoPanel extends JPanel {
				...
				public void run() {
					...
					while (i < iterationMax) {
						if (!isShowing) {
							...
							try {
								...
								/*
								 * If you find collectData is throwing an
								 * exception, instead catch that exception and
								 * store the value as
								 * BarChartRenderer.ERROR_CODE
								 */
								e.printStackTrace();
								return;
							}
					...
					if (data == null)
						throw new NullPointerException("null data for "
								+ ShowcaseChartDemoPanel.this.getClass()
										.getName());

					System.out.println(ShowcaseChartDemoPanel.this.getClass()
							.getSimpleName() + ":\n" + toHtml(data));
		...
		protected String toHtml(Map<String, Map<String, Long>> data) {
			...
			for (Entry<String, Map<String, Long>> entry : data.entrySet()) {
				...
				for (Entry<String, Long> entry2 : entry.getValue().entrySet()) {
					...
					String v = entry2.getValue().equals(
							BarChartRenderer.ERROR_CODE) ? "Error"
		 ...
		 * @return the iteration index split into separate parameters, or null
		 *         if the iterationIndex is too large to split into parameters
		 *         based on paramSize
		 */
	...
	public JPanel createPanel(PumpernickelShowcaseApp psa) {
		return new ShowcaseChartDemoPanel();
	}
}