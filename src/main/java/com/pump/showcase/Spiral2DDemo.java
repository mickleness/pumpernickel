 ...
 */
public class Spiral2DDemo implements ShowcaseDemo {
	static class Spiral2DDemoPanel extends JPanel {
		...
		JSpinner coilOffset = new JSpinner(
				new SpinnerNumberModel(0, 0, 10, .01));
			...
			MouseInputAdapter mouseListener = new MouseInputAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					double coilGapValue = ((Number) coilGap.getValue())
							.doubleValue();
					double coilsValue = ((Number) coils.getValue())
							.doubleValue();
					...
					if (e.isShiftDown()) {
						...
						spiral = Spiral2D.createWithFixedCoilGap(centerX,
								centerY, endX, endY, coilGapValue);
					}
					setSpiral(spiral);
				}
				...
				if (showFlattenedSpiral.isSelected()) {
					...
					for (double t = 0; t < coilsValue; t += .01) {
						...
						double x = spiral.getCenterX()
								+ r
								* Math.cos(m * t * Math.PI * 2
										+ angleOffsetValue);
						double y = spiral.getCenterY()
								+ r
								* Math.sin(m * t * Math.PI * 2
										+ angleOffsetValue);
