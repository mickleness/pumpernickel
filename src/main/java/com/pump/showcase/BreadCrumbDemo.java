 ...
 */
public class BreadCrumbDemo implements ShowcaseDemo {
	static class BreadCrumbDemoPanel extends JPanel {
		...
		static class RoundedShadowIcon implements Icon {
			...
			public RoundedShadowIcon() {
				...
				g2.setPaint(new GradientPaint(0, 0, darkShadow, scratch
						.getWidth() - 3, 0, lightShadow));
				g2.fillRect(0, 0, scratch.getWidth(), scratch.getHeight());
		...
		private static DefaultMutableTreeNode add(
				DefaultMutableTreeNode parent, String childName) {
		...
		public BreadCrumbDemoPanel() {
			...
			breadCrumb.setBorder(new CompoundBorder(new LineBorder(
					Color.lightGray), new EmptyBorder(0, 4, 0, 4)));
		...
		private void refreshBreadCrumb() {
			...
			/*
			 * Give it a small preferred size so the GridBagLayout chooses to
			 * expand it on its own terms.
			 * 
			 * If we don't do this: sometimes the width will exceed the
			 * available space. This is OK for the JBreadCrumb, but it affects
			 * other areas of the UI. In this demo's case: when this
			 * BreadCrumbDemo panel has a preferred size that exceeds its
			 * available size, then its container suffers and the JSeparator
			 * above it disappears. (Which is weird, because the deficit is
			 * horizontal, so why are we giving up vertical space?).
			 * 
	...
	public JPanel createPanel(PumpernickelShowcaseApp psa) {
		return new BreadCrumbDemoPanel();
	}
