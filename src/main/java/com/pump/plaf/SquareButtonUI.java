 ...
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/filledbuttonui/SquareButtonUI.png"
...
public class SquareButtonUI extends QButtonUI {
	...
	public static final SimpleButtonFill SQUARE_FILL = new SimpleButtonFill() {
		...
		private Color[] normalColors = new Color[] { new Color(0xFFF4F4F4),
				new Color(0xFFDDDDDD), new Color(0xFFF9F9F9),
				new Color(0xFFF9F9F9) };

		private Color[] darkestColors = new Color[] { new Color(0xFFB9B9B9),
				new Color(0xFF727272), new Color(0xFFCBCBCB),
				new Color(0xFFCBCBCB) };

		private Color[] darkerColors = new Color[] { new Color(0xFFBEBEBE),
				new Color(0xFF838383), new Color(0xFFDCDCDC),
	...
	public void paintBackground(Graphics2D g, ButtonInfo info) {
		...
		if (info.button.getClientProperty(SHAPE) == null
				&& info.button.isContentAreaFilled()) {
			g.setStroke(new BasicStroke(1));
			boolean showSeparators = isShowingSeparators(info.button);
			g.setColor(shadow);
			Rectangle rect = new Rectangle(info.fillBounds.x,
					info.fillBounds.y, info.fillBounds.width,
			...
			if (showSeparators) {
				...
				if (verticalPosition == POS_ONLY) {
					...
					if (horizontalPosition == POS_LEFT) {
						partialRect.moveTo(info.fillBounds.x
								+ info.fillBounds.width, rect.y);
						partialRect.lineTo(rect.x, rect.y);
						partialRect.lineTo(rect.x, rect.y + rect.height);
						partialRect.lineTo(info.fillBounds.x
								+ info.fillBounds.width, rect.y + rect.height);
					} else if (horizontalPosition == POS_MIDDLE) {
						partialRect.moveTo(info.fillBounds.x
								+ info.fillBounds.width, rect.y);
						partialRect.lineTo(info.fillBounds.x, rect.y);
						partialRect.moveTo(info.fillBounds.x, rect.y
								+ rect.height);
						partialRect.lineTo(info.fillBounds.x
								+ info.fillBounds.width, rect.y + rect.height);
