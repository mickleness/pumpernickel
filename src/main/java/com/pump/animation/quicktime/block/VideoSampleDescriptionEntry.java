...
/**
 * This is the SampleDescriptionEntry for a VideoSampleDescriptionBlock.
 */
public class VideoSampleDescriptionEntry extends SampleDescriptionEntry {
	...
	public VideoSampleDescriptionEntry(InputStream in) throws IOException {
		...
		temporalQuality = Block.read32Int(in);
		spatialQuality = Block.read32Int(in);
		width = Block.read16Int(in);
		height = Block.read16Int(in);
		horizontalResolution = Block.read16_16Float(in);
		verticalResolution = Block.read16_16Float(in);
		dataSize = Block.read32Int(in);
		frameCount = Block.read16Int(in);
		compressorName = Block.read32BytePascalString(in);
		depth = Block.read16Int(in);
		colorTableID = Block.read16Int(in);
	}
	...
	protected void write(OutputStream out) throws IOException {
		...
		Block.write32Int(out, temporalQuality);
		Block.write32Int(out, spatialQuality);
		Block.write16Int(out, width);
		Block.write16Int(out, height);
		Block.write16_16Float(out, horizontalResolution);
		Block.write16_16Float(out, verticalResolution);
		Block.write32Int(out, dataSize);
		Block.write16Int(out, frameCount);
		Block.write32BytePascalString(out, compressorName);
		Block.write16Int(out, depth);
		Block.write16Int(out, colorTableID);
	}
