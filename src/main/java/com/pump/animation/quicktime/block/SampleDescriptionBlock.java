...
/**
 * The sample description block stores information that allows you to decode
 ...
 * {@link SoundSampleDescriptionBlock} and {@link VideoSampleDescriptionBlock}
 * <p>
 * The sample description block has an block type of 'stsd'. The sample
 * description block contains a table of sample descriptions. A media may have
 ...
 * the data. The sample-to-chunk block identifies the sample description for
 * each sample in the media by specifying the index into this table for the
 * appropriate description (see {@link SampleToChunkBlock}).
 */
public class SampleDescriptionBlock extends LeafBlock {

	/** "stsd" */
	public static final String BLOCK_TYPE = "stsd";

	protected int version = 0;
	protected int flags = 0;
	protected SampleDescriptionEntry[] entries = new SampleDescriptionEntry[0];

	public SampleDescriptionBlock(int version, int flags) {
		super(null);
		this.version = version;
		this.flags = flags;
	}

	public SampleDescriptionBlock() {
		super(null);
	}

	public SampleDescriptionBlock(Block parent, InputStream in)
			throws IOException {
		super(parent);
		version = in.read();
		flags = read24Int(in);
