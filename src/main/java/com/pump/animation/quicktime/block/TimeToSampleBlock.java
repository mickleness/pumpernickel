...
/**
 * Time-to-sample blocks store duration information for a media’s samples,
 * providing a mapping from a time in a media to the corresponding data sample.
 * The time-to-sample block has an block type of 'stts'.
 * <p>
 * You can determine the appropriate sample for any time in a media by examining
 * the time-to-sample block table, which is contained in the time-to-sample
 * block.
 * <p>
 * The block contains a compact version of a table that allows indexing from
 * time to sample number. Other tables provide sample sizes and pointers from
 * the sample number. Each entry in the table gives the number of consecutive
 ...
 * <p>
 * The block contains time deltas: <code>DT(n+1) = DT(n) + STTS(n)</code> where
 ...
 * any edit list). The edit list block provides the initial DT value if it is
 * nonempty (nonzero).
 */
public class TimeToSampleBlock extends LeafBlock {
	...
	public static class TimeToSampleEntry {
		...
		public TimeToSampleEntry(InputStream in) throws IOException {
			sampleCount = Block.read32Int(in);
			sampleDuration = Block.read32Int(in);
		}
		...
		protected void write(OutputStream out) throws IOException {
			Block.write32Int(out, sampleCount);
			Block.write32Int(out, sampleDuration);
		}
	}

	/** "stts" */
	public static final String BLOCK_TYPE = "stts";

	protected int version = 0;
	protected int flags = 0;
	protected TimeToSampleEntry[] table = new TimeToSampleEntry[0];

	public TimeToSampleBlock(int version, int flags) {
		super(null);
		this.version = version;
		this.flags = flags;
	}

	public TimeToSampleBlock() {
		super(null);
	}

	public TimeToSampleBlock(Block parent, InputStream in) throws IOException {
		super(parent);
		version = in.read();
		flags = read24Int(in);
	...
	public String toString() {
		...
		return "TimeToSampleBlock[ version=" + version + ", " + "flags="
				+ flags + ", " + "table=" + tableString + "]";
	}

	/**
	 * Add a new sample time to this block.
	 * 
	 * @param duration
	...
	/**
	 * Return a 1-byte specification of the version of this time-to-sample
	 * block.
	 */
