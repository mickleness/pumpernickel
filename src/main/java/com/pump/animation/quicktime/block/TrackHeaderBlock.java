...
/**
 * The track header block specifies the characteristics of a single track within
 * a movie. A track header block contains a size field that specifies the number
 * of bytes and a type field that indicates the format of the data (defined by
 * the block type 'tkhd').
 */
public class TrackHeaderBlock extends LeafBlock {
	...
	/** "tkhd" */
	public static final String BLOCK_TYPE = "tkhd";
	...
	public TrackHeaderBlock(long trackID, long duration, float width,
			float height) {
		super(null);
	...
	public TrackHeaderBlock(Block parent, InputStream in) throws IOException {
		super(parent);
		version = in.read();
		flags = read24Int(in);
	...
	public String toString() {
		return "TrackHeaderBlock[ version=" + version + ", " + "flags=" + flags
	 ...
	 * each 'tkhd' block of the other tracks in the group. QuickTime chooses one
