...
/**
 * You use sample size blocks to specify the size of each sample in the media.
 * Sample size blocks have an block type of 'stsz'.
 * <p>
 * The sample size block contains the sample count and a table giving the size
 * of each sample. This allows the media data itself to be unframed. The total
 ...
 */
public class SampleSizeBlock extends LeafBlock {

	/** "stsz" */
	public static final String BLOCK_TYPE = "stsz";
	...
	public SampleSizeBlock(int version, int flags, long sampleSize,
	...
	public SampleSizeBlock() {
		super(null);
		sizeTable = new long[0];
	}

	public SampleSizeBlock(Block parent, GuardedInputStream in)
			throws IOException {
		super(parent);
		version = in.read();
		flags = read24Int(in);
	...
	public String toString() {
		...
		return "SampleSizeBlock[ version=" + version + ", " + "flags=" + flags
				+ ", " + "sampleSize=" + sampleSize + ", sampleCount = "
				+ sampleCount + ", " + "sizeTable=" + entriesString + "]";
	}

	/**
	 * Return a 1-byte specification of the version of this sample size block.
	 */
