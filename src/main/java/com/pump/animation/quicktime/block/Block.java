...
/**
 * A block ("atom") in a QuickTime file.
 * <p>
 ...
 */
public abstract class Block implements TreeNode {
	/**
	...
	/**
	 * Returns the complete size of this block, including the 8 byte header that
	 ...
	 * 
	 * @return the complete size of this block.
	 */
	...
	/**
	 * Returns the 4-byte identifier this block uses. These identifiers are
	 * often defined in the QT file format.
	 * <P>
	 * (Although this uses a <code>java.lang.String</code>, serious badness will
	 * follow if this value is not exactly 4-bytes long.
	 * 
	 * @return the 4-byte identifier this block uses (such as "moov", "trak",
	 *         etc.)
	 */
	public abstract String getBlockType();

	/**
	 * Writes the contents of this block, minus the first 8 bytes. This is
	 * called from the <code>write()</code> method.
	 * <P>
	 * (The first 8 bytes are always the size of this block, and its identifier.
	...
	/**
	 * Write this block to an OutputStream.
	 * <p>
	...
	public final void write(OutputStream out) throws IOException {
		if (this.getClass().equals(EmptyBlock.class)) {
			write32Int(out, 0);
			return;
		}
		long size = getSize();
		write32Int(out, size);
		String id = getBlockType();
		...
		if (out2.getLimit() != 0) {
			...
			throw new IOException("This block is " + out2.getLimit()
					+ " byte(s) too small.");
		}
	}
	...
	public Block getChild(Class<?> t) {
		for (int a = 0; a < getChildCount(); a++) {
			Block block = getChildAt(a);
			if (t.isInstance(block))
				return block;
		}
		return null;
	}
	 ...
	 * <code>TrackHeaderBlocks</code> and determine the highest track ID used in
	 * this movie
	 * 
	...
	public long getHighestTrackID() {
		long maxID = 1;
		if (this instanceof TrackHeaderBlock) {
			TrackHeaderBlock t = (TrackHeaderBlock) this;
			if (t.trackID > maxID)
				maxID = t.trackID;
		}
		for (int a = 0; a < getChildCount(); a++) {
			Block block = getChildAt(a);
			long k = block.getHighestTrackID();
			if (k > maxID)
				maxID = k;
		}
		return maxID;
	...
	@Override
	public abstract Block getChildAt(int index);

	@Override
	public String toString() {
		return getClass().getSimpleName() + "[ type=\"" + getBlockType()
				+ "\", size=" + getSize() + "]";
	}
}