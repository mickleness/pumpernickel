...
public class BlurbIndexBuilder {
	...
	class BlurbWriteup implements Comparable<BlurbWriteup> {
		...
		public String getHTML() {
			...
			stringWriter
					.append("<table border=\"0\" cellspacing=\"0px\" cellpadding=\"10px\" padding=\"0px\" width=\"650px\">\n");
			stringWriter.append("\t<tr>\n");
			stringWriter
					.append("\t\t<td style=\"background: linear-gradient( rgb(255, 255, 255), rgb(241, 241, 241));border:1px solid rgb(220,220,220);width=100%;height=100%;box-shadow: 0px 1px 3px #888888;\">\n");
			...
			if (article == null || article.length() == 0) {
				stringWriter
						.append("\t\t\t\t\t<td><span style=\"font-weight: bold;font-size:110%;}\">"
								+ title + "</span>");
			} else {
				stringWriter
						.append("\t\t\t\t\t<td><span style=\"font-weight: bold;font-size:110%;}\"><a href=\""
			...
			// this block writes the upper-right corner of the table which may
			// include
			// the jar link and/or the jnlp link
			{
				// this preserves the order the links are presented in (this
				// stores the
				...
				stringWriter
						.append("\t\t\t\t\t<td style=\"text-align:right;vertical-align:text-top;\"><span style=\"font-weight: bold;}\">");
				Class blurbClass = blurb.getBlurbClass();
				String javadocUrl;
				if (blurbClass.getName().endsWith("package-info")) {
					javadocUrl = "https://mickleness.github.io/pumpernickel/javadoc/"
							+ blurbClass.getName().replace(".", "/")
									.replace("package-info", "package-summary")
							+ ".html";
				} else {
					javadocUrl = "https://mickleness.github.io/pumpernickel/javadoc/"
							+ blurbClass.getName().replace(".", "/") + ".html";
				}
				stringWriter.append("<a href=\"" + javadocUrl
						+ "\">Javadoc</a>");
			...
			if (!b.imageName().isEmpty()) {
				...
				if (!blurbGraphicFile.exists())
					throw new RuntimeException(
							blurbGraphicFile.getAbsolutePath()
									+ " does not exist.");
				...
				if (blurbGraphicFile != null && blurbGraphicFile.exists()
						&& imageSize != null) {
					stringWriter
							.append("<img align=\"right\" style=\"padding:8px\" src=\"blurbs/"
									+ blurbGraphicFile.getName()
									+ "\" width=\""
									+ imageSize.width
									+ "\" height=\""
									+ imageSize.height
	...
	public void run(Workspace workspace) throws IOException {
		...
		if (!blurbDirectory.exists()) {
			throw new RuntimeException(blurbDirectory.getAbsolutePath()
					+ " does not exist.");
		}

		FileTreeIterator iter = new FileTreeIterator(workspace.getDirectory(),
				"java");
		while (iter.hasNext()) {
		...
		String indexText = templateString.replace("<!insertTable>",
				sb.toString());
