...
/**
 * A set of jar files identified by {@link JarId} keys. This is derived from the
 * local file system and modified by build processes.
 */
public class Releases {
	 ...
	 * 
	 * @param jarId
	 *            the jar ID being assigned.
	 * @param file
	 *            if null then this file is marked as removed/missing.
	 */
	...
	public void save() throws IOException {
		...
		for (Entry<JarId, File> entry : map.entrySet()) {
			...
			if (!versionDir.exists())
				versionDir.mkdirs();
			String jarName = entry.getKey().artifactId + "-"
					+ entry.getKey().version + ".jar";
			...
			if (Workspace.DELETED_FILE.equals(srcFile)) {
				...
				if (targetFile.exists()
						&& IOUtils.zipEquals(targetFile, srcFile,
								".*pom\\.properties")) {
					System.out.println("Skipped "
							+ targetFile.getAbsolutePath() + " (no changes)");
					continue;
				}
				if (targetFile.exists()) {
