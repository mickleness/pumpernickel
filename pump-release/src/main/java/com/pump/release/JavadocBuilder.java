...
public class JavadocBuilder {
	...
	class JavaFile {
		...
		JavaFile(File file) throws Exception {
			this.file = file;

			Token[] tokens = new JavaParser().parse(IOUtils.read(file), false);
	...
	private void buildSourcePath(Workspace workspace, File tempSourcePath)
			throws Exception {
		...
		while (iter.hasNext()) {
			...
			if ("pump-release".equals(javaFile.artifactId)
					...
					|| javaFile.file.getAbsolutePath().contains(
	...
	public void run(Workspace workspace) throws Exception {
		...
		if (!tempSourcePath.mkdirs())
			throw new IOException("File.mkdirs failed for "
					+ tempSourcePath.getAbsolutePath());
		...
		String cmd = "javadoc -J-mx140m -d " + javadocDest.getCanonicalPath()
				+ " -classpath " + srcPathsString + File.pathSeparator
				+ System.getProperty("java.class.path") + " " + packageNames;
		...
		if (!docsDir.exists())
			throw new RuntimeException(docsDir.getAbsolutePath()
					+ " does not exist");
		File javadocDir = new File(docsDir, "javadoc");
		if (!javadocDir.exists())
			throw new RuntimeException(javadocDir.getAbsolutePath()
					+ " does not exist");

		stripDateInfo(javadocDest);
	...
	private void stripDateInfo(File javadocDest) throws IOException {
		for (File htmlFile : new FileTree(javadocDest, "html")) {
			// <!-- Generated by javadoc (1.8.0_45) on Sun Dec 03 20:32:43 EST
			// 2017 -->
			// <title>EditAtom</title>
			// <meta name="date" content="2017-12-03">

			String str = IOUtils.read(htmlFile);
			str = removeTag(str, "<!-- Generated by javadoc");
			str = removeTag(str, "<meta name=\"date\"");
			IOUtils.write(htmlFile, str, true);
		}
	}

	private String removeTag(String htmlBody, String searchPhrase) {
		int i = htmlBody.indexOf(searchPhrase);
		if (i > -1) {
			int i2 = htmlBody.indexOf(">", i + 1);
			return htmlBody.substring(0, i) + htmlBody.substring(i2 + 1);
		}
