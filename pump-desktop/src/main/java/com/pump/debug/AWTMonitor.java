...
public class AWTMonitor {
	 ...
	 * @param addPanicDialogPrompt if true then the <code>PanicDialogPrompt</code> is added.
	 */
	public synchronized static void installAWTListener(String applicationName,long stackTraceDelay,final long panicListenerDelay,boolean addPanicDialogPrompt) {
		if(addPanicDialogPrompt)
			addPanicListener(defaultPrompt);
		if(awtMonitorThread==null || awtMonitorThread.isAlive()==false) {
			awtMonitorThread = createAWTMonitorThread(applicationName, stackTraceDelay, panicListenerDelay, "AWT Listener (Debug Tool)", System.err);
	 ...
	 */
	static Thread createAWTMonitorThread(final String applicationName, final long stackTraceDelay, final long panicListenerDelay, final String threadName,final PrintStream dest) {
							...
							while(awtRunnable.flag==false) {
								...
								if(reportedToConsole==false && current-start>stackTraceDelay) {
									if(reportNeeded==-1) {
										...
										dumpThreads("The AWT thread was unresponsive for "+stackTraceDelay/1000+" seconds.  Here is a stack trace from all available threads:", dest);
	 ...
	 * @param headerText text to print that precedes the
	 * stack traces. 
	 * @param dest the PrintStream to write data to.
	 */
	public static void dumpThreads(String headerText, PrintStream dest) {
		try {
			...
			if(JVM.getMajorJavaVersion()>=1.5) {
				...
				dest.println("Stack traces were requested, but this feature is not supported in this Java version ("+JVM.getMajorJavaVersion()+").");
			}
		}
	}
}