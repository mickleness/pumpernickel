...
public class ColorDemo extends JPanel {
	...
	enum PaletteStyle {
		Solid() {
			public void run(ColorPalette palette) {
				palette.putClientProperty(ColorSet.PALETTE_STYLE_PROPERTY,
	...
	enum PaletteCellStyle {
		None() {
			public void run(ColorPalette palette) {
				palette.putClientProperty(ColorSet.PALETTE_CELL_STYLE_PROPERTY,
	...
	public static class ColorPaletteDemo extends JPanel {
		...
		JTextArea description = new JTextArea(
				"The ColorPalette component is a grid presenting a variety of colors the user can select. It is not as compact as the ColorWell component, but it is not as large as a typical color-choosing dialog.");
		JPanel controls = new JPanel(new GridBagLayout());
		JPanel paletteContainer = new JPanel(new GridBagLayout());
		ColorPalette colorPalette = new ColorPalette();

		JComboBox<PaletteType> typeComboBox = createComboBox(PaletteType.class,
				new ActionListener() {
					@Override
		...
		JComboBox<PaletteStyle> styleComboBox = createComboBox(
				PaletteStyle.class, new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						PaletteStyle style = (PaletteStyle) styleComboBox
								.getSelectedItem();
						style.run(colorPalette);
					}
				});

		JComboBox<PaletteCellStyle> cellStyleComboBox = createComboBox(
				PaletteCellStyle.class, new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						PaletteCellStyle style = (PaletteCellStyle) cellStyleComboBox
								.getSelectedItem();
						style.run(colorPalette);
					}
				});

		private <T> JComboBox<T> createComboBox(Class<T> type,
	...
	Section colorPickerSection = container.addSection("colorPicker",
			"Color Picker");
	Section colorPaletteSection = container.addSection("colorPalette",
			"Color Palette");
	...
	public ColorDemo() {
		...
		container.getHeader(colorPickerSection).putClientProperty(
				CollapsibleContainer.COLLAPSIBLE, Boolean.FALSE);
		container.getHeader(colorPaletteSection).putClientProperty(
				CollapsibleContainer.COLLAPSIBLE, Boolean.FALSE);
	}
