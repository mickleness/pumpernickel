...
public abstract class StrokeDemo extends JPanel implements ChangeListener {
	...
	public static Rectangle2D defineWiggle(GeneralPath dest) {
		...
		for (int a = 0; a < segments; a++) {
			...
			minY = Math.min(minY,
					Math.min(y1, Math.min(y2, Math.min(y3, Math.min(y4, y5)))));
			maxY = Math.max(maxY,
					Math.max(y1, Math.max(y2, Math.max(y3, Math.max(y4, y5)))));
		}
		Rectangle2D bounds = new Rectangle2D.Float(0, minY, segments * 4, maxY
				- minY);
		return bounds;
	}
	...
	JPanel previewPanel = new JPanel() {
		...
		protected void paintComponent(Graphics g0) {
			...
			for (float a = 0; a < k; a += 20 + width) {
				e.setFrame(10 + a, 10 + a, d.width - 20 - 2 * a, d.height - 20
						- 2 * a);
			...
			for (int a = 0; a < rings.length; a++) {
				...
				for (int b = 0; b < p.length; b++) {
					...
					if (p[b].equals(Integer.TYPE) || p[b].equals(Integer.class)) {
						obj[b] = new Integer(
								((Number) spinners[b].getValue()).intValue()
										+ extra);
					} else if (p[b].equals(Float.TYPE)
							|| p[b].equals(Float.class)) {
						obj[b] = new Float(
								((Number) spinners[b].getValue()).floatValue()
										+ extra);
					} else if (p[b].equals(Double.TYPE)
							|| p[b].equals(Double.class)) {
						obj[b] = new Double(
								((Number) spinners[b].getValue()).doubleValue()
										+ extra);
					} else if (p[b].equals(Long.TYPE)
							|| p[b].equals(Long.class)) {
						obj[b] = new Long(
								((Number) spinners[b].getValue()).longValue()
										+ extra);
					}
				}
				try {
			 ...
			 * 
			 * for(int a = 0; a<rings.length; a++) { g.setStroke(new
			 * BasicStroke(.5f));
	...
	/**
	 * 
	 * @param theStrokeClass
	 *            the type of stroke.
	 * @param theConstructor
	 *            the constructor to invoke.
	 * @param labels
	 *            the textual descriptions of the spinners.
	 * @param spinners
	 *            one spinner for every argument the constructor needs.
	 */
	public StrokeDemo(Class<?> theStrokeClass, Constructor<?> theConstructor,
			JLabel[] labels, JSpinner[] spinners) {
