 ...
 * alt="new&#160;com.bric.geom.ClipperDemo()"> <!-- ======== END OF
...
public class ClipperDemo extends JPanel {
	...
	/**
	 * Measure the time and the memory performance of the Clipper class vs. the
	 * Area class.
	 */
	public static void runNumbers(PrintStream dest) {
		...
		for (int a = 0; a < p.length; a++) {
			...
			// since this takes a while, let's mention to the console that we're
			// doing something:
		...
		Arrays.sort(t1);
		Arrays.sort(t2);
		dest.println("Using an Area class took: " + t1[t1.length / 2] + " ms, "
				+ m1[m1.length / 2] + " bytes");
		dest.println("Using clipToRect took: " + t2[t2.length / 2] + " ms, "
				+ m2[m2.length / 2] + " bytes");
	}
