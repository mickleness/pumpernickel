...
public abstract class MultiWindowDemo extends JPanel {
	...
	/**
	 * When this client property on the root pane is attached to a String, that
	 * String is displayed when the mouse hovers over that root pane.
	 */
	public static final String KEY_DESCRIPTION = SwingComponentsDemo.class
			.getName() + "#description";

	/**
	 * Maps to a GridBagConstraints used for tile positioning.
	 */
	private static final String KEY_CONSTRAINTS = SwingComponentsDemo.class
			.getName() + "#constraints";

	protected JDesktopPane desktop = new JDesktopPane();
	protected JPanel bottomPanel = new JPanel(new GridBagLayout());
	protected MagnificationPanel zoomPanel = new MagnificationPanel(desktop,
			20, 20, 8);
		...
		MouseTracker.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				if (desktop.isShowing()) {
					Point p = new Point(MouseTracker.getX(), MouseTracker
							.getY());
					SwingUtilities.convertPointFromScreen(p, desktop);
					Component c0 = SwingUtilities.getDeepestComponentAt(
							desktop, p.x, p.y);
					String description = "";
					if (c0 instanceof JComponent) {
						JComponent c = (JComponent) c0;
						description = (String) c
								.getClientProperty(KEY_DESCRIPTION);
	 ...
	 * @param gridheight
	 * @param fill
	 *            GridBagConstraints.HORIZONTAL, VERTICAL, BOTH or NONE. This is
	 *            only applied if the JDesktopPane is resizable.
	 */
	protected void addPane(JInternalFrame pane, int gridx, int gridy,
	...
	protected void layoutWindows() {
		...
		for (Entry<JInternalFrame, GridBagConstraints> entry : map.entrySet()) {
			...
			// apply insets
			r.setBounds(r.x + insets.left, r.y + insets.top, r.width
					- insets.left - insets.right, r.height - insets.top
					- insets.bottom);
	...
	private void setConstraint(JComponent jc, int gridx, int gridy, int width,
			int height, int fill) {
		if (!(fill == GridBagConstraints.BOTH
				|| fill == GridBagConstraints.HORIZONTAL
				|| fill == GridBagConstraints.VERTICAL || fill == GridBagConstraints.NONE))
			throw new IllegalArgumentException(
					"fill should be BOTH, HORIZONTAL, VERTICAL, or NONE");
		GridBagConstraints c = new GridBagConstraints();
		c.gridx = gridx;
		c.gridy = gridy;
		c.gridwidth = width;
