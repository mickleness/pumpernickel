...
/**
 * This simple app makes a few comparison related to whether the ResourcePool
 * marks an improvement or not.
 * 
 **/
public class ResourcePoolDemo extends OutputDemo {
	...
	protected String runImageTest(boolean useCache, boolean paint,
			boolean clearImage) {
		...
		for (int a = 0; a < times.length; a++) {
			...
			for (int b = 0; b < 50; b++) {
				BufferedImage bi;
				if (useCache) {
					bi = ResourcePool.get().getImage(1000, 1000,
							BufferedImage.TYPE_INT_ARGB, clearImage);
				} else {
	...
	protected void runScalingLongTest(PrintStream out, PrintStream err,
			PrintStream blue) {
		long[] times = new long[20];
		out.println("\nScaling Test:");
