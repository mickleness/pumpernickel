...
public class GifWriterDemo extends JPanel {
	...
	static class ImageAnimationReader implements ResettableAnimationReader {
		...
		public ImageAnimationReader(List<BufferedImage> allImages,
				double duration) {
		...
		public BufferedImage getNextFrame(boolean cloneImage)
			...
			g.drawImage(k, bi.getWidth() / 2 - k.getWidth() / 2, bi.getHeight()
					/ 2 - k.getHeight() / 2, null);
			index++;
			g.dispose();
	...
	static class DemoAnimationReader implements ResettableAnimationReader {
		...
		DemoAnimationReader(int width, int height) {
			...
			this.h = height;
			int k = 64;
			BufferedImage patternImage = new BufferedImage(k, k,
			...
			transparentPattern = new TexturePaint(patternImage, new Rectangle(
					0, 0, k, k));
		}

		public BufferedImage getNextFrame(boolean cloneImage)
				throws IOException {
			if (ctr == getFrameCount())
				return null;
			...
			float r = (float) (Math.sqrt(rect.getWidth() * rect.getWidth()
					+ rect.getHeight() * rect.getHeight())) / 2f;
			Ellipse2D e = new Ellipse2D.Float(bi.getWidth() / 2 - r,
					bi.getHeight() / 2 - r, 2 * r, 2 * r);
			...
			g.drawString("" + ctr,
					(float) (bi.getWidth() / 2 - rect.getWidth() / 2),
					(float) (bi.getHeight() / 2 + rect.getHeight() / 2 - fm
							.getDescent()));
			g.dispose();
			ctr++;
			return bi;
	...
	public GifWriterDemo() {
		...
		layout.addRow(new JLabel("Frame Duration:"),
				combine(durationField, new JLabel("seconds")), false);
	...
	protected void write(File dir) {
		...
		if (images.size() == 0) {
			Frame frame = (Frame) SwingUtilities
					.getWindowAncestor(GifWriterDemo.this);
			QDialog.showDialog(frame, "Error", QDialog.ERROR_MESSAGE,
					"No images were found.",
					"No images were found in the folder \"" + dir.getName()
							+ "\".", null, // innerComponent,
		...
		try {
			...
			throw new RuntimeException("The duration \""
		...
		try {
			GifWriter.write(destFile,
					new ImageAnimationReader(images, duration),
					GifWriter.ColorReduction.FROM_ALL_FRAMES);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected void writeDefault() {
		File destFile = getDestinationFile();
		try {
			GifWriter.write(destFile, new DemoAnimationReader(400, 300),
					GifWriter.ColorReduction.FROM_FIRST_FRAME);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
