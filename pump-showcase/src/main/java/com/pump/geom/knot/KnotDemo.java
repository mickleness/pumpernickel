...
/**
 * This explores how to render some celtic knots. (A reader wrote with some
 * questions.)
 *
 * 
 ...
 * <p>
 * <img src=
 * "https://raw.githubusercontent.com/mickleness/pumpernickel/master/pump-release/resources/samples/KnotApplet/sample.png"
 * alt="new&#160;com.pump.geom.knot.KnotApplet()">
 * <!-- ======== END OF AUTOGENERATED SAMPLES ======== -->
 */
@ResourceSample(sample = "new com.pump.geom.knot.KnotApplet()")
public class KnotDemo extends JPanel {
	...
	class PreviewPanel extends JPanel {
		...
		protected SortedSet<Intersection> getIntersections(RenderedShape shape) {
			SortedSet<Intersection> dest = new TreeSet<Intersection>(
					new ShapeComparator(shape));
		...
		protected void paintComponent(Graphics g) {
			...
			for (int a = 0; a < shapes.length; a++) {
				...
				while (iter.hasNext()) {
					...
					if (!processedIntersections.contains(intersection)) {
						if (ctr % 2 == overValue) {
							intersection.shape1.paint(g, intersection.x,
									intersection.y, intersection.shape2, debug);
							intersection.shape1Over = Boolean.TRUE;
						} else {
							intersection.shape2.paint(g, intersection.x,
									intersection.y, intersection.shape1, debug);
							intersection.shape1Over = Boolean.FALSE;
						}
						processedIntersections.add(intersection);
						processedIntersections
								.add(intersection.createInverse());
					}
					ctr++;
				}
			}
			...
			if (debug) {
				...
				while (iter.hasNext()) {
					Intersection intersection = iter.next();
					g.fillOval((int) (intersection.x - 3),
							(int) (intersection.y - 3), 6, 6);
				}
			}
		}
	}

	RenderedShape shape1 = new RenderedShape(new Ellipse2D.Float(40 + 20,
			40 + 20, 200, 200), new Color(0x888899));
	RenderedShape shape2 = new RenderedShape(new RoundRectangle2D.Float(
			100 + 20, 0 + 20, 80, 280, 4, 4), new Color(0x889988));
	RenderedShape shape3 = new RenderedShape(new RoundRectangle2D.Float(0 + 20,
			100 + 20, 280, 80, 4, 4), new Color(0x998888));
